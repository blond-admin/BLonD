...
directory = 'output_figs_max_jitter_minus'
...
if tracking:
    ...
    gif_paths = [
        '/Users/lvalle/PycharmProjects/BLonD/Simulations_ramps/Z_mode/simu_acceleration_positron/gif_path/animated_ramp_Z_mode_jitter.gif',
    ]
    ...
    opmode = 'Z'
    folder_name = opmode + '_mode/frames_jitter'
    os.makedirs("frames_jitter", exist_ok=True)
    filenames = []
    for i in range(1, Nturns + 1):
        ...
        if get_data_animation:
            if i < 50:
                frame_path = f"frames_jitter/plot_{i:03d}.png"
    ...
    for filename in filenames:
        if os.path.exists(filename):
            os.remove(filename)
    if os.path.exists("frames_jitter") and not os.listdir("frames_jitter"):
        os.rmdir("frames_jitter")

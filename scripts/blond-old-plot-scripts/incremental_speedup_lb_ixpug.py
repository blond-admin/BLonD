...
gconfig = {
    ...
    'fontsize': 10,
    'legend': {
        'loc': 'upper left', 'ncol': 5, 'handlelength': 1.7, 'fancybox': True,
    ...
    'ylim': [0, 70],
    'yticks': [0, 20, 40, 60],
    'outfiles': ['{}/{}-{}-incremental-speedup-o20-ixpug.pdf',
                 '{}/{}-{}-incremental-speedup-o20-ixpug.jpg']
}
...
if __name__ == '__main__':
    for title, figconf in lconfig['figures'].items():
        ...
        for col, case in enumerate(args.cases):
            ...
            for idx, k in enumerate(final_d.keys()):
                speedup = 100 * final_d[k][1] / final_d['Base'][1] - 100
                x = final_d[k][0]
                label = k
            ...
            plt.xticks(np.arange(len(x)) + 2.5 * width, np.array(x, int)//20)

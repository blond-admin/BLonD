...
gconfig = {
    ...
    # 'hatches': ['', '', 'xx', '', 'xx', '', 'xx', '', 'xx'],
    'colors': ['xkcd:grey', 'xkcd:orange', 'xkcd:sky blue'],
    # 'colors': {
    #     'f64-cpu': 'xkcd:black',
    ...
    'ticks': {'fontsize': 10},
    'xticks': {'rotation': 90, 'fontsize': 10},
    'fontsize': 10,
    'legend': {
        'loc': 'upper left', 'ncol': 1, 'handlelength': 1.5, 'fancybox': True,
        'framealpha': .9, 'fontsize': 10, 'labelspacing': 0, 'borderpad': 0.5,
    ...
    for col, case in enumerate(args.cases):
        ...
        for model, color in zip([cpu_model, base_gpu_model, gpu_model],
            ...
            for idx, k in enumerate(gconfig['label'].values()):
                ...
                plt.bar(pos, speedup, width=0.8*width,

...
gconfig = {
    ...
    'label': {
        'double-exact-gpu0': 'HBLonD',
        # 'tp-double-exact-gpu0': 'f64-cpu-tp',
        # 'tp-double-exact-gpu0': 'HBLonD',
        'single-exact-gpu0': 'HBLonD-f32',
        # 'double-exact-gpu0': 'base',
        # 'double-exact-gpu1': 'f64-gpu',
        'double-exact-gpu1': 'CuBLonD',
        'double-exact-gpu2': 'CuBLonD-2GPU',
        # 'single-exact-gpu1': 'f32',
        'single-exact-gpu1': 'CuBLonD-f32',
    ...
    'colors': {
        'f64-cpu': 'xkcd:black',
        'HBLonD': 'xkcd:orange',
        'HBLonD-f32': 'xkcd:orange',
        'CuBLonD': 'xkcd:sky blue',
        'CuBLonD-2GPU': 'xkcd:sky blue',
        'CuBLonD-f32': 'xkcd:sky blue',
        'f32-SRP-2': 'xkcd:red',
        'f32-SRP-3': 'xkcd:red',
    ...
    'fontsize': 10,
    'legend': {
        'loc': 'upper left', 'ncol': 1, 'handlelength': 1.1, 'fancybox': True,
        'framealpha': 0.7, 'fontsize': 9, 'labelspacing': 0, 'borderpad': 0.1,
        'handletextpad': 0.3, 'borderaxespad': 0.1, 'columnspacing': 0.8,
    ...
    'tick_params_left': {
        ...
        'direction': 'inout', 'length': 3, 'width': 1,
    },
...
if __name__ == '__main__':
    ...
    for col, case in enumerate(args.cases):
        ...
        idx = 0
        # for idx, k in enumerate(sorted(plots_dir.keys())):
        for k in gconfig['label'].values():
            ...
            plt.bar(pos + np.arange(len(x)), speedup,
                    width=0.95*width,
                    linewidth=1.,
                    label=label,
                    edgecolor='black',
            ...
            #             xy=(pos + len(x)-1, speedup[-1]), va='bottom',
            #             rotation='0', **gconfig['annotate'])
            pos += 1 * width
            idx += 1
        # pos += width * step
        # plt.xticks(np.arange(len(x)), np.array(x, int)//20, **gconfig['ticks'])
        plt.xticks(np.arange(len(x))+(pos-width)/2, np.array(x, int)//20, **gconfig['ticks'])

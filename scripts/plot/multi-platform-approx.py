...
parser.add_argument('-i', '--inputdirs', type=str, nargs='+',
                    help='The input directories, one for every platform. The last is the base platform.')

parser.add_argument('-m', '--models', type=str, nargs='+',
                    help='The platform names. The last is the base platform.')
...
gconfig = {
    ...
    'colors': ['xkcd:black', 'xkcd:red', 'xkcd:blue',
               'xkcd:orange', 'xkcd:green', 'xkcd:brown'],
    # 'colors': {
    #     'f64-cpu': 'xkcd:black',
...
if __name__ == '__main__':
    for col, case in enumerate(args.cases):
        ...
        xtickspos = []
        labels = set()
        for model, color in zip([cpu_model] + gpu_models,
                                gconfig['colors']):
        ...
        for file in gconfig['outfiles']:
            models = this_filename[:-3]+'_'.join(args.models)
            file = file.format(images_dir, models, case)

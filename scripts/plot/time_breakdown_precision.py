...
if __name__ == '__main__':
    for col, case in enumerate(args.cases):
        ...
        fig, ax_arr = plt.subplots(ncols=len(plots_dir), nrows=1,
        ...
        for idx, k in enumerate(plots_dir.keys()):
            ...
            plt.title(f'{case.upper()}-{precision}', **gconfig['title'])

            for row in plots_dir[k]:
                func = row[header.index(gconfig['x_name'])]
                for cat, subcat in gconfig['categories'].items():
                    if func in subcat:
                        if cat not in final_dir:
                            final_dir[cat] = 0
                        final_dir[cat] += float(row[header.index(gconfig['y_name'])])

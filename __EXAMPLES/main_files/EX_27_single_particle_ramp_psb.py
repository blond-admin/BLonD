...
ring = Ring(
    C,
    alpha,
    np.linspace(sync_momentum, sync_momentum + momentum_increase, N_t + 1),
    Proton(),
    n_turns=N_t,
)
...
# --- RF tracker object ---
blond_tracker = RingAndRFTracker(
    rfstation,
    beam,
    Profile=None,
    interpolation=False,
...
Y_phi_s = np.sqrt(
    abs(
        -np.cos(rfstation.phi_s[0])
        + (np.pi - 2 * rfstation.phi_s[0]) / 2 * np.sin(rfstation.phi_s[0])
    )
)
dE_sep = (
    np.sqrt(
        2
        * abs(rfstation.charge)
        * rfstation.voltage[0, 0]
        * rfstation.beta[0]
        * rfstation.beta[0]
        * rfstation.energy[0]
        / (np.pi * rfstation.harmonic[0, 0] * abs(ring.eta_0[0, 0]))
    )
    * Y_phi_s
)
...
bunchmonitor = BunchMonitor(
    ...
    rfstation,
    beam,
    results_directory + "/EX_27_output_data",
    Profile=None,
)
...
plots = Plot(
    ...
    Profile=None,
    h5file=results_directory + "/EX_27_output_data",
...
line.particle_ref = xp.Particles(
    p0c=sync_momentum, mass0=xp.PROTON_MASS_EV, q0=1.0
)
line.build_tracker()
...
line.track(
    particles, num_turns=N_t, turn_by_turn_monitor=True, with_progress=True
)
...
test_string += (
    "{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}"
    "\t{:+10.10e}\t{:+10.10e}\n"
).format(
    beam.dE[0],
    beam.dt[0],
    rfstation.momentum[0],
    rfstation.gamma[0],
    rfstation.beta[0],
    rfstation.energy[0],
    mon.x[0, 0].T,
...
xsuite_string += (
    "{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n".format(
        mon.ptau[0, 0].T,
        mon.zeta[0, 0].T,
        np.float64(mon.p0c[0, 0]),
        np.float64(mon.gamma0[0, 0]),
...
for i in range(N_t):
    dt, dE = xsuite_to_blond_transform(
        ...
        rfstation.beta[i],
        rfstation.energy[i],
        phi_s=rfstation.phi_s[i]
        - rfstation.phi_rf[0, 0],  # Correct for RF phase
    ...
    xsuite_string += (
        "{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n".format(
            mon.ptau[0, i].T,
            mon.zeta[0, i].T,
            np.float64(mon.p0c[0, i]),
            np.float64(mon.gamma0[0, i]),

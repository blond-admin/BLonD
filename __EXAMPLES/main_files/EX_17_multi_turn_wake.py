...
os.makedirs(this_directory + "../output_files/EX_17_fig/", exist_ok=True)
...
slice_beam.n_macroparticles = (
    n_macroparticles
    * slice_beam.bin_size
    / (sigma_dt * np.sqrt(2.0 * np.pi))
    * np.exp(
        -0.5
        * (slice_beam.bin_centers - bucket_length / 2.0) ** 2.0
        / sigma_dt**2.0
    )
)
...
total_ind_volt_freq_periodic = TotalInducedVoltage(
    beam, slice_beam, [ind_volt_freq_periodic]
)
...
plt.plot(
    ...
    lw=2,
    label="Z in time MTW in freq",
    alpha=0.75,
)
plt.plot(
    slice_beam.bin_centers * 1e9,
    total_ind_volt_freq_periodic.induced_voltage,
    label="Z in freq. MTW from periodicity",
    lw=2,
    alpha=0.75,
)
...
time_array = np.arange(
    -np.sum(RF_sct_par.t_rev[1:]), bucket_length, slice_beam.bin_size
)

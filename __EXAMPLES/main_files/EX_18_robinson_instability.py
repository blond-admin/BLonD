...
try:
    os.mkdir('../output_files/EX18_fig')
except:
    pass
...
plt.savefig('../output_files/EX18_fig/impedance.png')
plt.close()
...
for i in range(n_turns):
    ...
    if i % n_turns_between_two_plots == 0:
        ...
        plt.savefig('../output_files/EX18_fig/phase_space_{0:d}.png'.format(i))
        plt.close()
...
plt.savefig('../output_files/EX18_fig/bunch_center.png')
plt.close()
plt.figure()
plt.plot(bunch_std*1e9)
plt.xlabel('Turns')
plt.ylabel('Bunch length [ns]')
plt.savefig('../output_files/EX18_fig/bunch_length.png')
plt.close()

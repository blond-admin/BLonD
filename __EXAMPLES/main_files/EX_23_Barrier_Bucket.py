...
##################################################################
bbgen_centered = bbuck.BarrierGenerator(
    pos_centered, barrier_width, barrier_amplitude
)
bbgen_edges = bbuck.BarrierGenerator(
    pos_edges, barrier_width, barrier_amplitude
)
...
centered_terms = bbgen_centered.for_rf_station(
    times=[0], t_rev=[t_rev], harmonics=barrier_harmonics, m=2
)
offset_terms = bbgen_edges.for_rf_station(
    times=[0], t_rev=[t_rev], harmonics=barrier_harmonics, m=2
)
...
for i in range(len(barrier_harmonics)):
    print(
        f"h={centered_terms[0][i]}, V={centered_terms[1][i][1, 0] / 1e3:.2f} kV "
...
for i in range(len(barrier_harmonics)):
    print(
        f"h={offset_terms[0][i]}, V={offset_terms[1][i][1, 0] / 1e3:.2f} kV "
...
centered_waveform = bbuck.harmonics_to_waveform(
    barrier_bins, centered_terms[0], centered_amplitudes, centered_phases
)

offset_waveform = bbuck.harmonics_to_waveform(
    barrier_bins, offset_terms[0], offset_amplitudes, offset_phases
)

plt.plot(
    barrier_bins * 1e9,
    centered_waveform / 1e3,
    color="blue",
    lw=3,
    label="Centered Barrier",
)
...
for i in range(ring.n_turns):
    ...
    if i % 1000 == 0:
        ...
        plt.gca().twinx().plot(
            barrier_bins * 1e9, offset_waveform / 1e3, color="red"
        )
        plt.show()

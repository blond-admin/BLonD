...
Y_phi_s = np.sqrt(
    abs(
        -np.cos(rfstation_1.phi_s[0])
        + (np.pi - 2 * rfstation_1.phi_s[0]) / 2 * np.sin(rfstation_1.phi_s[0])
    )
)
dE_sep = (
    np.sqrt(
        2
        * abs(rfstation_1.charge)
        * V
        * rfstation_1.beta[0]
        * rfstation_1.beta[0]
        * rfstation_1.energy[0]
        / (np.pi * rfstation_1.harmonic[0, 0] * abs(ring.eta_0[0, 0]))
    )
    * Y_phi_s
)
...
bunchmonitor = BunchMonitor(
    ...
    rfstation_1,
    beam,
    results_directory + "/EX_25_output_data",
    Profile=None,
)
...
plots = Plot(
    ...
    Profile=None,
    h5file=results_directory + "/EX_25_output_data",

...
sigma_dt = (
    np.abs(ring.eta_0[0, 0])
    / (ring.beta[0, 0] * ring.energy[0, 0])
    * sigma_E
    / rf_station.omega_s0[0]
)

profile = Profile(
    beam,
    cut_options=CutOptions(
        cut_left=-5 * sigma_dt, cut_right=5 * sigma_dt, n_slices=n_bins
    ),
)
...
haissinski_solution = Haissinski(
    ring_tracker, iSR, verbose=True, seed=seed, root_kwargs={"method": "hybr"}
)
...
ax_h.plot(
    ...
    haissinski_solution.x * beam.intensity * e,
    "C1",
    label="Haissinski",
)
ax_h.plot(

...
phase_loop = BeamFeedback_PSB(general_params, rf_params, slices_ring,
                              PL_gain = 0, RL_gain=[1.e7, 1.e11], period = 10.e-6)
...
bunch_monitor = BunchMonitor(general_params, rf_params, my_beam,
                             this_directory + '../output_files/EX_09_updt_output_data',
                             Profile=slices_ring, PhaseLoop=phase_loop)


# Plots
format_options = {'dirname': this_directory + '../output_files/EX_09_updt_fig'}
plots = Plot(general_params, rf_params, my_beam, 100, n_turns, 0.0, 2.0 * 0.9e-6,
             ...
             h5file=this_directory + '../output_files/EX_09_updt_output_data', PhaseLoop=phase_loop)
...
for i in range(1, n_turns + 1):
    ...
    if i == 100:
        phase_loop.radial_reference += 0.00001  # /1.6e-6
...
test_string += '{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n'.format(
    np.mean(my_beam.dE), np.std(my_beam.dE), np.mean(my_beam.dt), np.std(my_beam.dt))
with open(this_directory + '../output_files/EX_09_updt_test_data.txt', 'w') as f:
    f.write(test_string)

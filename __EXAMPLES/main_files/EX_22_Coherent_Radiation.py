...
Z_pp.imped_calc(
    freqs, low_frequency_transition=1e-4, high_frequency_transition=10
)
...
plt.tight_layout()
plt.savefig(
    this_directory + "../output_files/EX_22_fig/csr_impedance_real.png"
)
...
plt.tight_layout()
plt.savefig(
    this_directory + "../output_files/EX_22_fig/csr_impedance_imag.png"
)
...
print(
    f"energy loss per turn (integrated spectrum): {energy_loss / e_charge:1.3f} eV"
)
...
profile = Profile(
    ...
    CutOptions=CutOptions(
        cut_left=-6 * sigma_dt, cut_right=6 * sigma_dt, n_slices=128
    ),
)

# create the CSR impedance sources
cSR_source_fs = CoherentSynchrotronRadiation(r_bend, gamma=gamma)
cSR_source_pp = CoherentSynchrotronRadiation(
    r_bend, chamber_height=h, gamma=gamma
)
...
freq_res = 0.01e12  # [Hz]
cSR_Z_fs = InducedVoltageFreq(
    beam, profile, [cSR_source_fs], frequency_resolution=freq_res
)
cSR_Z_pp = InducedVoltageFreq(
    beam, profile, [cSR_source_pp], frequency_resolution=freq_res
)
cSR_Z_fs_appr = InducedVoltageFreq(
    beam, profile, [cSR_source_fs_appr], frequency_resolution=freq_res
)
cSR_Z_pp_appr = InducedVoltageFreq(
    beam, profile, [cSR_source_pp_appr], frequency_resolution=freq_res
)
...
plt.plot(
    W_fs.time_array * 1e12, W_fs.induced_voltage, ".-", label="free-space"
)
plt.plot(W_pp.time_array * 1e12, W_pp.induced_voltage, label="parallel-plates")
plt.plot(
    W_fs_appr.time_array * 1e12,
    W_fs_appr.induced_voltage,
    label="free-space, approx.",
)
plt.plot(
    W_pp_appr.time_array * 1e12,
    W_pp_appr.induced_voltage,
    "--",
    label="parallel-plates, approx.",
)
plt.legend()
...
plt.plot(
    profile.bin_centers * 1e12,
    profile.n_macroparticles / beam.n_macroparticles,
    "grey",
)
plt.tight_layout()
...
for it, t in enumerate(times):
    w_fs[it] = np.trapezoid(
        Z_fs.impedance * Lambda * np.exp(2j * np.pi * freqs * t), freqs
    ).real
    w_pp[it] = np.trapezoid(
        Z_pp.impedance * Lambda * np.exp(2j * np.pi * freqs * t), freqs
    ).real
    w_fs_appr[it] = np.trapezoid(
        Z_fs_appr.impedance * Lambda * np.exp(2j * np.pi * freqs * t), freqs
    ).real
    w_pp_appr[it] = np.trapezoid(
        Z_pp_appr.impedance * Lambda * np.exp(2j * np.pi * freqs * t), freqs
    ).real
# convert to volt
for tmp in [w_fs, w_pp, w_fs_appr, w_pp_appr]:

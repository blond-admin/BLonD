...
# Phase loop
phase_loop = BeamFeedbackPSB(general_params, rf_params, slices_ring, PL_gain=0., gain_radial_loop=[0., 0.], dt=10.0e-6)
...
plots = Plot(general_params, rf_params, my_beam, 1000, 10000, 0.0, 2.0 * 0.9e-6,
             ...
             h5file=this_directory + '../gpu_output_files/EX_10_output_data', phase_loop=phase_loop)
...
test_string += '{:+10.10e}\t{:+10.10e}\t{:+10.10e}\t{:+10.10e}\n'.format(
    np.mean(my_beam.dE), np.std(my_beam.dE), np.mean(my_beam.dt), np.std(my_beam.dt))
with open(this_directory + '../gpu_output_files/EX_10_test_data.txt', 'w') as f:
    f.write(test_string)

...
CL.track()
logging.info('Antenna voltage is %.4f MV', np.mean(np.absolute(CL.V_ANT[-10:]))*1.e-6)
...
logging.info('Updated generator current is %.4f A', np.mean(np.absolute(CL.I_gen)))
P_gen = CL.generator_power()
logging.info('Generator power is %.4f kW', np.mean(P_gen)*1e-3)

...
# Printouts
def logging_info(OTFB, no_cavities):
    logging.info(
        "  Generator resistance R_gen %.8f kOhms", OTFB.TWC.R_gen / 1e3
    )
    voltage = np.average(np.absolute(OTFB.V_ANT_COARSE[-10]) * no_cavities)
    logging.info("  Final voltage cavities %.8e V" % voltage)
    current = np.average(np.absolute(OTFB.I_GEN_COARSE[-10])) * OTFB.n_cavities
    logging.info("  Final generator current %.8e A" % current)
    logging.info(
        "  Calculated resistance %.8f kOhms" % (voltage / current / 1e3)
    )


# Cavity-group pre-tracking
def init_otfb(no_sections, no_cavities, V_part, G_tx, df):
...
Commissioning = SPSCavityLoopCommissioning(
    debug=False, open_loop=False, open_fb=True, open_drive=False, open_ff=True
)
...
if PRE_LS2_5SEC:
    ...
    OTFB = init_otfb(
        5, 2, 5 / 9, 1.00066011, df=rf.omega_rf[0, 0] / (2 * np.pi) - 200.1e6
    )
    logging_info(OTFB, 2)
...
logging.info(
    "Final voltage, 4-section cavities %.8e V"
    % np.average(
        np.absolute(OTFB.OTFB_1.V_ANT_COARSE) * OTFB.OTFB_1.n_cavities
    )
)
logging.info(
    "Final voltage, 5-section cavities %.8e V"
    % np.average(
        np.absolute(OTFB.OTFB_2.V_ANT_COARSE) * OTFB.OTFB_2.n_cavities
    )
)
...
OTFB = SPSCavityFeedback(
    ...
    commissioning=Commissioning,
    df=[
        rf.omega_rf[0, 0] / (2 * np.pi) - 200.03766667e6,
        rf.omega_rf[0, 0] / (2 * np.pi) - 199.9945e6,
    ],
)
logging.info(
    "Final voltage, 3-section cavities %.8e V"
    % np.average(
        np.absolute(OTFB.OTFB_1.V_ANT_COARSE) * OTFB.OTFB_1.n_cavities
    )
)

...
tau_0 = 0.31e-9         # Initial bunch length, 4 sigma [s]
...
CL = LHCCavityLoop(
    rf, profile, f_c=rf.omega_rf[0, 0]/(2 * np.pi), G_gen=1,
    I_gen_offset=0, n_cavities=8, n_pretrack=100, Q_L=20_000,
    R_over_Q=R_over_Q, tau_loop=650e-9, tau_otfb=1200e-9,
    RFFB=LHCCavityLoopCommissioning(open_loop=False, open_otfb=False, G_a=6.8e-6, G_d=10, G_o=10,
                                    excitation=False)
)
...
CL = LHCCavityLoop(
    rf, profile, f_c=rf.omega_rf[0, 0]/(2 * np.pi) + d_f, G_gen=1,
    I_gen_offset=0, n_cavities=8, n_pretrack=1000, Q_L=20_000,
    R_over_Q=R_over_Q, tau_loop=650e-9, tau_otfb=1200e-9,
    RFFB=LHCCavityLoopCommissioning(alpha=15/16, open_loop=False, open_otfb=False,
                                    G_a=6.8e-6, G_d=10, G_o=10, mu=-10)
)
CL.rf_beam_current()
...
plt.plot(np.real(CL.I_BEAM_COARSE[CL.n_coarse:]), 'b', alpha=0.5, label='real')
plt.plot(np.imag(CL.I_BEAM_COARSE[CL.n_coarse:]), 'r', alpha=0.5, label='imag')
plt.plot(np.absolute(CL.I_BEAM_COARSE[CL.n_coarse:]), 'g', alpha=0.5, label='ampl')
plt.xlabel('Samples [at 40 MS/s]')
plt.ylabel('RF beam current [A]')
plt.legend()
plt.tight_layout()
...
logging.info('Total DC beam current %.4e A', np.sum(CL.profile.n_macroparticles)
             / beam.n_macroparticles*beam.intensity * e / ring.t_rev[0])

logging.info('Maximum RF beam current %.4e A', np.max(np.absolute(CL.I_BEAM_COARSE)))
...
logging.info('Final generator current is %.10f A', np.mean(np.absolute(CL.I_GEN_COARSE[-10:])))
...
ax1.plot(np.absolute(CL.V_ANT_COARSE) * 1e-6, 'b', linewidth=0.3)
ax1.set_xlabel('Samples [at 40 MS/s]')
ax1.set_ylabel('Antenna voltage [MV]')
ax2 = fig.add_subplot(gs[1, 0:2], sharex=ax1)
ax2.plot(np.angle(CL.V_ANT_COARSE, deg=True), 'b', linewidth=0.3)
ax2.set_xlabel('Samples [at 40 MS/s]')
ax2.set_ylabel('Phase [degrees]')
ax3 = fig.add_subplot(gs[:, 2:4])
ax3.scatter(CL.V_ANT_COARSE.real * 1e-6, CL.V_ANT_COARSE.imag * 1e-6)
...
gs = plt.GridSpec(2, 4)
ax1 = fig.add_subplot(gs[0, 0:2])
ax1.plot(np.absolute(CL.I_GEN_COARSE), 'b', linewidth=0.3)
ax1.set_xlabel('Samples [at 40 MS/s]')
ax1.set_ylabel('Generator current [A]')
ax2 = fig.add_subplot(gs[1, 0:2], sharex=ax1)
ax2.plot(np.angle(CL.I_GEN_COARSE, deg=True), 'b', linewidth=0.3)
ax2.set_xlabel('Samples [at 40 MS/s]')
ax2.set_ylabel('Phase [degrees]')
ax3 = fig.add_subplot(gs[:, 2:4])
ax3.scatter(CL.I_GEN_COARSE.real, CL.I_GEN_COARSE.imag)

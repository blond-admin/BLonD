...
rf = RFStation(
    ring, [h], [3.1e6], [dphi]
)  # SPS-equivalent for 0.57 eVs, 1.65 ns
...
for i in range(12, 60):
    beam.dt[i * N_m : (i + 1) * N_m] = (
        bunch.dt[0:N_m] + i * buckets + 32 * buckets
    )
    beam.dE[i * N_m : (i + 1) * N_m] = bunch.dE[0:N_m]
for i in range(60, 108):
    beam.dt[i * N_m : (i + 1) * N_m] = (
        bunch.dt[0:N_m] + i * buckets + 40 * buckets
    )
    beam.dE[i * N_m : (i + 1) * N_m] = bunch.dE[0:N_m]
for i in range(108, 156):
    beam.dt[i * N_m : (i + 1) * N_m] = (
        bunch.dt[0:N_m] + i * buckets + 48 * buckets
    )
    beam.dE[i * N_m : (i + 1) * N_m] = bunch.dE[0:N_m]
for i in range(156, 500):
    beam.dt[i * N_m : (i + 1) * N_m] = (
        bunch.dt[0:N_m] + i * buckets + 200 * buckets
    )
    beam.dE[i * N_m : (i + 1) * N_m] = bunch.dE[0:N_m]
...
CL = LHCCavityLoop(
    ...
    RFFB=LHCCavityLoopCommissioning(
        open_loop=False, open_otfb=True, G_a=6.8e-6, G_d=10, excitation=False
    ),
)
if PLOT_NO_BEAM:
...
d_f = LHCCavityLoop.half_detuning(
    rf.omega_rf[0, 0] / (2 * np.pi), I_rf_pk, R_over_Q, V / 8
)
...
CL = LHCCavityLoop(
    rf,
    profile,
    f_c=rf.omega_rf[0, 0] / (2 * np.pi) + d_f,
    G_gen=1,
    I_gen_offset=0,
    n_cavities=8,
    n_pretrack=100,
...
plt.plot(
    np.real(CL.I_BEAM_COARSE[CL.n_coarse :]), "b", alpha=0.5, label="real"
)
plt.plot(
    np.imag(CL.I_BEAM_COARSE[CL.n_coarse :]), "r", alpha=0.5, label="imag"
)
plt.plot(
    np.absolute(CL.I_BEAM_COARSE[CL.n_coarse :]), "g", alpha=0.5, label="ampl"
)
plt.xlabel("Samples [at 40 MS/s]")
plt.ylabel("RF beam current [A]")
plt.legend()
...
logging.info(
    "Total DC beam current %.4e A",
    np.sum(CL.profile.n_macroparticles)
...
logging.info(
    "Maximum RF beam current %.4e A", np.max(np.absolute(CL.I_BEAM_COARSE))
)
...
logging.info(
    "Final generator current is %.10f A",
    np.mean(np.absolute(CL.I_GEN_COARSE[-10:])),
)
P_gen = CL.generator_power()
logging.info(
    "Average generator power before beam injection is %.10f kW",
    np.mean(P_gen) * 1e-3,
)
...
P_gen = CL.generator_power()
logging.info(
    "Average generator power after beam injection is %.10f kW",
    np.mean(P_gen) * 1e-3,
)

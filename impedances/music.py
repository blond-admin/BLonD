...
class Music(object):
    ...
    def __init__(self, Beam, resonator, n_macroparticles, n_particles, t_rev):
        ...
        self.induced_voltage[0] = np.sum(self.const)/2
        ...
        self.input_first_component = np.ones(len(self.R_S))
        self.input_second_component = np.zeros(len(self.R_S))
        self.t_rev = t_rev
        self.last_dt = self.beam.dt[-1]
        self.array_parameters = np.array([self.t_rev, self.last_dt])

    def track_cpp(self):

        libblond.music_track(self.beam.dt.ctypes.data_as(ctypes.c_void_p),
                             self.beam.dE.ctypes.data_as(ctypes.c_void_p),
                             self.induced_voltage.ctypes.data_as(
                                 ctypes.c_void_p),
                             self.array_parameters.ctypes.data_as(
                                 ctypes.c_void_p),
                             self.input_first_component.ctypes.data_as(
                                 ctypes.c_void_p),
                             self.input_second_component.ctypes.data_as(
                                 ctypes.c_void_p),
                             ctypes.c_int(len(self.beam.dt)),
                             ctypes.c_int(self.n_resonators),
                             self.alpha.ctypes.data_as(ctypes.c_void_p),
                             self.omega_bar.ctypes.data_as(ctypes.c_void_p),
                             self.const.ctypes.data_as(ctypes.c_void_p),
                             self.coeff1.ctypes.data_as(ctypes.c_void_p),
                             self.coeff2.ctypes.data_as(ctypes.c_void_p),
                             self.coeff3.ctypes.data_as(ctypes.c_void_p),
                             self.coeff4.ctypes.data_as(ctypes.c_void_p))
    ...
    def track_py(self):
        ...
        for i in range(len(self.beam.dt)-1):
            ...
            self.induced_voltage[i+1] = np.sum(self.const *
        ...
        self.last_dt = self.beam.dt[-1]
        self.beam.dE += self.induced_voltage

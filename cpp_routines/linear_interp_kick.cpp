...
// Optimised routine that calculates the kick of a voltage array on particles
...
extern "C" void linear_interp_kick(double * __restrict__ beam_dt,
    ...
    // Constant factor taking into account the time distance between 
    // consecutive bins
    const double inv_bin_width = (n_slices-1) / 
                          (bin_centers[n_slices-1] - bin_centers[0]);
    // Constant factor taking into account the particles' charge and the 
    // factor above
    const double factor = charge * inv_bin_width;
    ...
    for (int i = 0; i < n_macroparticles; i++) {
        ...
        if ((a < bin_centers[0])||(a > bin_centers[n_slices-1]))
            ...
            voltageKick = voltage_array[ffbin] + (a - bin_centers[ffbin]) * 
                        (voltage_array[ffbin+1]-voltage_array[ffbin]) * factor;

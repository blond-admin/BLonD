...
def calculate_phi_s(RFStation, Particle=Proton(),
    ...
    * 'first': not yet implemented. Its purpose should be to adjust the
    ...
    if accelerating_systems == 'as_single':
        ...
        return calc_phi_s_as_single(Particle, RFStation, eta0)

    elif accelerating_systems == 'all':

        return calc_phi_s_all(Particle, RFStation, eta0)
...
def calc_phi_s_all(Particle, RFStation, eta0):
    ...
    for indexTurn in range(len(RFStation.delta_E)):
        ...
        potential_well = - cumtrapz(
            np.sign(eta0[indexTurn]) * (totalRF - RFStation.delta_E[indexTurn] / abs(Particle.charge)),
            dx=float(phase_array[1] - phase_array[0]),
            initial=0)
...
def calc_phi_s_as_single(Particle, RFStation, eta0):

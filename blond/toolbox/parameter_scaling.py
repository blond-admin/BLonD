...
circumferences = {'0': 2 * np.pi * 25,
                  '1': 2 * np.pi * 100.,
                  '2': 2 * np.pi * 1100.009, '3': 2 * np.pi * 1100.009,
                  '4': 2 * np.pi * 1100.009,
...
class ParameterScaling(object):
    ...
    def frequencies(self):
        ...
        self.tb1.append(
            "    RF frequency: " + np.str(self.f_RF * 1.e-6) + " MHz")
    ...
    def tune(self):
        ...
        self.tb1.append(
            "    Central synchrotron tune: " + np.str(self.Q_s0) + "")
    ...
    def bucket_parameters(self):
        ...
        self.tb1.append(
            "Bucket parameters assume: single RF, stationary case, and no "
            "intensity effects.\n")

        self.bucket_area = 8. * np.sqrt(
            2. * self.beta_sq * self.energy * self.voltage /
            (np.pi * self.harmonic * self.eta_0)) / self.omega_rf
        self.tb1.append(
            "    Bucket area: " + np.str(self.bucket_area) + " eVs")
        ...
        self.dE_max = np.sqrt(
            2. * self.beta ** 2 * self.energy * self.voltage /
    ...
    def bunch_length_from_emittance(self, emittance):
        ...
        self.tb1.append(
            "    Bunch length is: " + np.str(self.tau * 1.e9) + " ns")
        self.tb1.append(
            "    Corresponding matched rms relative momentum offset: " +
            np.str(self.delta_b) + "")
        self.tb1.append("    Emittance contour in phase: " +
                        np.str(self.phi_b) + " rad")
    ...
    def pbHandler(self):
        ...
        self.tb1.append(
            "    * with main harmonic: " + np.str(self.harmonic) + "")

...
def main(
    run_n_turns: int | None = None, n_macroparticles: int = int(1e4)
) -> None:
    momentum = 3.9051e9
    ...
    main_rf_station = MultiHarmonicRFStation(
        n_harmonics=3, main_harmonic_idx=0
    )
    ...
    sim.run_simulation(
        beams=(beam,),
        n_turns=n_turns if run_n_turns is None else run_n_turns,

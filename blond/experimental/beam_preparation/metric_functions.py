...
def rms_emittance(density: NumpyArray | CupyArray, dt_grid: NumpyArray | CupyArray, dE_grid: NumpyArray | CupyArray):
    ...
    xms = np.sum(density * dt_grid**2.)
    ybar = np.sum(density * dE_grid)
    yms = np.sum(density * dE_grid**2.)
    xybar = np.sum(density * dt_grid * dE_grid)

    #combine into rms emittance and scale by dt and dE resolution
    rms_emittance = np.pi*np.sqrt((xms - xbar**2.)
                                   * (yms - ybar**2.)
                                   - (xybar - xbar*ybar)**2.)

    return rms_emittance

def q_percent_emittance(density: NumpyArray | CupyArray, dt_grid: NumpyArray | CupyArray, dE_grid: NumpyArray | CupyArray, q : float = 0.9) -> float:
    ...
    n_bins = np.argmin(np.abs(cumulative_array-q)) #figure out how many bins are necessary to contain a fraction q of distribution

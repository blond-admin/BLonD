...
def metric_fitter(
    ...
    # minimize
    ideal_free_parameter, is_converged = newton(
...
def generalized_bucket_filler(
    ...
    """Generalized method for generating density distributions
    for a hamiltonian.

    Notes
    -----
    Allows for a density distribution rho(H, value)
    with a free parameters value. This value is then
    automatically selected to make the generated bunches
    satisfy some metric that is also user defined.

    This method does not have functionality for
    identifying the wells. When the number of buckets
    is specified the time axis is simply split into
    n_buckets equal sized pieces. If the edges of
    these slices do not neatly align with the maxima
    of the potential, bunches may not be generated
    correctly. These bounds can be changed by setting
...
def bucket_fill_by_emittance_gaussian(
    ...
    """Method for generating gaussian distributed bunches
    of specific emittances. Makes use of the generalized
    bucket filler method

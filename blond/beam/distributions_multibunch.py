...
def matched_from_distribution_density_multibunch_old(beam, Ring, FullRingAndRF, distribution_options_list,
...
def matched_from_distribution_density_multibunch(beam, Ring, FullRingAndRF, distribution_options_list,
    ...
    *Function to generate a multi-bunch beam using the matched_from_distribution_density
    ...
    if np.sum(intensity_per_bunch) != beam.intensity:
        print(
            'WARNING !! The total intensity per bunch does not match the total intensity of the beam, the beam.intensity will be overwritten and change the beam.ratio')
        beam.intensity = np.sum(intensity_per_bunch)
        beam.ratio = beam.intensity / beam.n_macroparticles

    if np.sum(n_macroparticles_per_bunch) != beam.n_macroparticles:
        print(
            'WARNING !! The number of macroparticles per bunch does not match the total number of the beam, the beam.n_macroparticles will be overwritten and change the beam.ratio')
        beam.n_macroparticles = int(np.sum(n_macroparticles_per_bunch))
        beam.ratio = beam.intensity / beam.n_macroparticles
        beam.id = np.arange(1, beam.n_macroparticles + 1, dtype=int)
    ...
    for indexBunch in range(0, n_bunches):
        ...
        if 'emittance' in distribution_options:
            ...
            emittance = np.ascontiguousarray(emittance)
        else:
            emittance = None

        if 'bunch_length' in distribution_options:
            bunch_length = distribution_options['bunch_length'][indexBunch]
            bunch_length = np.ascontiguousarray(bunch_length)
        else:
            bunch_length = None
        ...
        if (TotalInducedVoltage is not None) and (indexBunch < n_bunches - 1):
            TotalInducedVoltageIteration.profile.track()
            TotalInducedVoltageIteration.induced_voltage_sum()

            left_edge = (bunch_position_buckets_list[indexBunch + 1] - bucket_tolerance) * bucket_size_tau
            right_edge = (bunch_position_buckets_list[indexBunch + 1] + 1 + bucket_tolerance) * bucket_size_tau
            indexes = (TotalInducedVoltageIteration.profile.bin_centers > left_edge) * (
                        TotalInducedVoltageIteration.profile.bin_centers < right_edge)
            tau_induced_voltage_next_bunch = TotalInducedVoltageIteration.profile.bin_centers[indexes]
            induced_voltage_next_bunch = TotalInducedVoltageIteration.induced_voltage[indexes]
            time_induced_voltage_next_bunch = (
                        tau_induced_voltage_next_bunch - bunch_position_buckets_list[indexBunch + 1] * bucket_size_tau)
            extraVoltageDict = {'time_array': time_induced_voltage_next_bunch,
                                'voltage_array': induced_voltage_next_bunch}

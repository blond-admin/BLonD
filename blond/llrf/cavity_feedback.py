...
class SPSCavityFeedback:
    ...
    def __init__(
        ...
        commissioning: Optional[
            list | SPSCavityLoopCommissioning
...
class LHCCavityLoop(CavityFeedback):
    ...
    def full_detuning_phase_modulation(self):
        error_func = (
            self.fd_alpha
            * (self.Q_L / self.omega_rf)
            * np.imag(
                self.I_GEN_COARSE[: self.n_coarse]
                * np.conj(self.V_ANT_COARSE[: self.n_coarse])
            )
        )

        error_func = error_func - np.mean(error_func)

        self.ADAPTIVE_PHASE_DERIVATIVE[self.n_coarse :] = (
            self.ADAPTIVE_PHASE_DERIVATIVE[-self.n_coarse :]
        )
        self.ADAPTIVE_PHASE_DERIVATIVE[-self.n_coarse :] = (
            self.ADAPTIVE_PHASE_DERIVATIVE[self.n_coarse :] + error_func
        )

        integrated_phase = (
            moving_average(self.ADAPTIVE_PHASE_DERIVATIVE, self.n_coarse)[
                -self.n_coarse :
            ]
            * self.n_coarse
            * self.T_s
        ...
        self.PHASE_MODULATION[: self.n_coarse] = self.PHASE_MODULATION[
            -self.n_coarse :
        ]
...
class FCCBoosterCavityLoop(CavityFeedback):
    r"""Cavity loop to regulate the RF voltage in the FCC ACS cavities.
    Adapted from the LHCCavityLoop class.
    ...
    profile : class
        Beam profile object
    n_cavities : int
        Number of cavities per beam; default is 112
    f_c : float
        Central cavity frequency [Hz]; default is 801573985.3775489 Hz
    G_gen : float
        Overall driver chain gain [1]; default is 1
    I_gen_offset : float
        Generator current offset [A]; default is 0
    n_pretrack : int
        Number of turns to pre-track without beam; default is 200
    Q_L : float
        Cavity loaded quality factor; default is 1e7
    R_over_Q : float
        Cavity R/Q [Ohm]; default is 315.2 Ohms
    tau_loop : float
        Total loop delay [s]; default is 700e-9 s
    tau_otfb : float
    ...
    def __init__(
        ...
        n_pretrack: int = 200,
        Q_L: float = 1e7,
        R_over_Q: float = 315.2,
        tau_loop: float = 700e-9,

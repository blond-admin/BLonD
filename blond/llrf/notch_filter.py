...
# FIRST METHOD
def impedance_notches(
    f_rev, frequencies, imp_source, list_harmonics, list_width_depth
):
    ...
    for i in list_harmonics:
        ...
        frequencies_remainder_array = np.append(
            ...
            np.array(
                [
                    i * f_rev - halfminorWidth,
                    i * f_rev,
                    i * f_rev + halfminorWidth,
                ]
            ),
        )
        Z_remainder_array = np.append(
            Z_remainder_array,
            np.array(
                [
                    locals()["y_" + str(i)] / minorDepth,
                    locals()["y_" + str(i)] / majorDepth,
                    locals()["y_" + str(i)] / minorDepth,
                ]
            ),
        )
    ...
    frequencies_remainder_array = np.append(
        frequencies_remainder_array, array_left
    )
    Z_remainder_array = np.append(Z_remainder_array, left_y)
    frequencies_remainder = np.append(
        frequencies_remainder, frequencies_remainder_array
    )

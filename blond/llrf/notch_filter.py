...
def impedance_notches(f_rev, frequencies, imp_source, list_harmonics, list_width_depth):
    ...
    for i in list_harmonics:
        ...
        exec("indices_" + str(
            i) + " = np.where((frequencies >= (i*f_rev-halfmajorWidth))&(frequencies <= (i*f_rev+halfmajorWidth)))[0]")
        indices_final = np.append(indices_final, locals()['indices_' + str(i)])
        frequencies_remainder_array = np.append(frequencies_remainder_array, np.array(
            [i * f_rev - halfminorWidth, i * f_rev, i * f_rev + halfminorWidth]))
        Z_remainder_array = np.append(Z_remainder_array, np.array(
            [locals()['y_' + str(i)] / minorDepth, locals()['y_' + str(i)] / majorDepth,
             locals()['y_' + str(i)] / minorDepth]))

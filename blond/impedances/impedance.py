    ...
    *Calculates the induced voltage of several resonators for arbitrary
    ...
    def __init__(
        ...
        if multi_turn_wake:
            ...
            for turn_ind in range(
                n_turns_calculation + 1
            ):  # zeroth turn + all previously non-decayed turns
    ...
    def process(self):
        ...
        if self.multi_turn_wake:
            ...
            potential_min_cav = (
                self.rf_params.phi_s[0] / self.rf_params.omega_rf[0, 0]
            )
            min_index = np.abs(
            ...
            # line density.
            self.time_array = self.profile.bin_centers
            self.mtw_memory = np.zeros(0)
            self.atLineDensityTimes = True
            self.n_time = len(self.time_array)
        self.induced_voltage = np.zeros(
            len(self.time_array), dtype=bm.precision.real_t, order="C"
        )

...
from blond.legacy.blond2.llrf.cavity_feedback import CavityFeedback
...
class FullRingAndRF:
    ...
    *Definition of the full ring and RF parameters in order to be able to have
    ...
    def __init__(self, ring_and_rf_section: list[RingAndRFTracker]):
        ...
        for ring_and_rf_section_element in self.ring_and_rf_section:
            self.ring_circumference += (
                ring_and_rf_section_element.rf_params.section_length
            )
    ...
    def RingAndRFSection_list(self):
        warnings.warn(
            "RingAndRFSection_list is deprecated, use ring_and_rf_section",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def potential_well_generation(
        self,
        turn: int = 0,
        n_points: int = int(1e5),
        ...
        for RingAndRFSectionElement in self.ring_and_rf_section:
            ...
            for rf_system in range(RF_params.n_rf):
                voltages = np.append(
                    voltages, RF_params.voltage[rf_system, turn]
                )
                omega_rf = np.append(
                    omega_rf, RF_params.omega_rf[rf_system, turn]
                )
                phi_offsets = np.append(
                    phi_offsets, RF_params.phi_rf[rf_system, turn]
                )
        ...
        if time_array is None:
            ...
            time_array = np.linspace(
                float(first_dt), float(last_dt), int(n_points)
            )

        self.total_voltage = np.sum(
            voltages.T * np.sin(omega_rf.T * time_array + phi_offsets.T),
            axis=0,
        )
...
class RingAndRFTracker:
    ...
    def __init__(
        ...
        if self.solver not in ["simple", "exact", "legacy"]:
            # SolverError
            raise RuntimeError(
                "ERROR in RingAndRFTracker: Choice of"
                + " longitudinal solver not recognised!"
            )
        if (
            self.rf_params.alpha_order > 1
        ):  # Force exact solver for higher orders of eta
            self.solver = "exact"
        ...
        if (self.interpolation is True) and (self.profile is None):
            # ProfileError
            raise RuntimeError(
                "ERROR in RingAndRFTracker: Please specify a"
        ...
        if (self.rf_params.empty is True) and (self.periodicity is True):
            # PeriodicityError
            raise RuntimeError(
                "ERROR in RingAndRFTracker: Empty RFStation"
        ...
        if (self.cavityFB is not None) and (
            not hasattr(self.cavityFB, "__iter__")
        ):
    ...
    def kick(self, beam_dt: NumpyArray, beam_dE: NumpyArray, index: int):
        ...
        bm.kick(
            beam_dt,
            beam_dE,
            self.rf_params.voltage[:, index],
            self.rf_params.omega_rf[:, index],
            self.rf_params.phi_rf[:, index],
            self.rf_params.particle.charge,
            self.rf_params.n_rf,
    ...
    def rf_voltage_calculation(self):
        ...
        """
        voltages = bm.ascontiguousarray(
            self.rf_params.voltage[:, self.counter[0]]
        )
        omega_rf = bm.ascontiguousarray(
            self.rf_params.omega_rf[:, self.counter[0]]
        )
        phi_rf = bm.ascontiguousarray(
            self.rf_params.phi_rf[:, self.counter[0]]
        )
        ...
        if self.cavityFB is not None:
            ...
            for ind, feedback in enumerate(self.cavityFB):
                if feedback is not None:
                    self.rf_voltage += (
                        ...
                        * feedback.V_corr
                        * bm.sin(
                            omega_rf[ind] * self.profile.bin_centers
                            + phi_rf[ind]
                            + feedback.phi_corr
                        )
                    )
                else:
                    self.rf_voltage += bm.rf_volt_comp(
                        voltages[ind : ind + 1],
                        omega_rf[ind : ind + 1],
                        phi_rf[ind : ind + 1],
            ...
            self.rf_voltage = bm.rf_volt_comp(
                voltages, omega_rf, phi_rf, self.profile.bin_centers
            )
    ...
    def track(self):
        ...
        if self.rf_params.phi_noise is not None:
            if self.noiseFB is not None:
                ...
                self.rf_params.phi_rf[:, turn] += self.rf_params.phi_noise[
                    :, turn
                ]
        ...
        if self.rf_params.phi_modulation is not None:
            ...
            self.rf_params.omega_rf[:, turn] += self.rf_params.phi_modulation[
                1
            ][:, turn]
    ...
    def _kickdrift_considering_periodicity(self, turn: int):
        ...
        self.indices_right_outside = bm.where(
            self.beam.dt > self.rf_params.t_rev[turn + 1]
        )[0]
        self.indices_inside_frame = bm.where(
            self.beam.dt < self.rf_params.t_rev[turn + 1]
        )[0]
        ...
        if len(self.indices_right_outside) > 0:
            ...
            self.beam.dt[self.indices_right_outside] -= self.rf_params.t_rev[
                turn + 1
            ]

...
class BeamFeedback:
    ...
    def __init__(
        ...
        if self.machine == "LHC":
            ...
            if self.gain2 != 0:
                ...
                self.lhc_a = 5.25 - self.rf_station.omega_s0 / (np.pi * 40.0)
                #: | *LHC Synchronisation loop time constant [turns]*
                self.lhc_t = (
                    2 * np.pi * self.rf_station.Q_s * np.sqrt(self.lhc_a)
                ) / np.sqrt(
                    1
                    + self.gain
                    / self.gain2
        ...
        if self.RFnoise is not None and (
    ...
    def beam_phase(self):
        ...
        *Beam phase measured at the main RF frequency and phase. The beam is
        ...
        if self.time_offset is None:
            ...
            coeff = bm.beam_phase(
                self.profile.bin_centers[indexes],
    ...
    def beam_phase_sharpWindow(self):
        ...
        *Beam phase measured at the main RF frequency and phase. The beam is
        ...
        if self.alpha != 0.0:
            indexes = bm.logical_and(
                (self.time_offset - np.pi / omega_rf)
                <= self.profile.bin_centers,
                self.profile.bin_centers
                <= (-1 / self.alpha + self.time_offset - 2 * np.pi / omega_rf),  # TODO : this comma is an addition, should it be there, or is it just formatting?
            )
        else:
        ...
        scoeff = bm.trapz(
            bm.sin(omega_rf * self.profile.bin_centers[indexes] + phi_rf)
            * self.profile.n_macroparticles[indexes],
            dx=self.profile.bin_size,
        )
        ccoeff = bm.trapz(
            bm.cos(omega_rf * self.profile.bin_centers[indexes] + phi_rf)
            * self.profile.n_macroparticles[indexes],
            dx=self.profile.bin_size,
        )
        ...
        *Phase difference between beam and RF phase of the main RF system.
        ...
        if self.cavity_feedback is not None:
            current_thres = self.current_thres * np.max(
                np.abs(self.cavity_feedback.I_BEAM_COARSE[-self.cavity_feedback.n_coarse:])
            )
            # voltage difference
            self.dphi = self.dphi + np.mean(self.cavity_feedback.gap_voltage_phase[
                np.abs(self.cavity_feedback.I_BEAM_COARSE[-self.cavity_feedback.n_coarse:]) > self.current_thres  # TODO: this is likely not correct and should use the variable above instead
            ])
    ...
    def radial_steering_from_freq(self):
        ...
        *Frequency and phase change for the current turn due to the radial steering program.*
        ...
        self.rf_station.dphi_rf_steering += (
            2.0
            * np.pi
            * self.rf_station.harmonic[:, counter]
            * (
    ...
    def SPS_RL(self):
        ...
        self.domega_dphi = -self.gain * self.dphi
        self.domega_dR = (
            -bm.sign(self.rf_station.eta_0[counter])
            * self.gain2

...
def reload_cpp_backend(  # NOQA: PLR0915
    ...
    class CppSpecials(Specials):
        ...
        def wake_from_pole_residue(
            ...
            Each pole carries a state that is read, advanced by one bin and
            then given the bin's charge, in that order. So the state a bin
            reads holds the history referenced one bin back and *not* the
            bin's own charge: the kernel covers lags of one bin or more, and
            the caller adds the self-bin term. This is what lets the residues
            carry the triangle bin-average ``((exp(p*dt) - 1) / (p*dt))**2``,
            whose lag likewise starts at ``dt`` and which stays bounded by one
            at any binning -- see `MultiPoleSparseSolve._finalize_solver`.

...
* **Caller-supplied build parameters** (compiler, ``native_tuning``,
  ``flags``, ``libs``, FFTW/Boost options) *are* folded in, with defaults that mirror
...
def build_options_valid(options: dict, expected_keys: Sequence[str]) -> bool:
    ...
    3. A syntax-only dry-run compile (``_check_dry_run_compile``) of
       the saved compiler with the saved ``native_tuning``/``flags``
       options, which catches a flag the compiler no longer accepts (e.g. after a
    ...
    dry_run_flags = []
    if options.get("native_tuning", True):

...
class BeamBaseClass(Preparable, ABC):
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        The particle arrays are always returned as NumPy arrays, whichever
        backend the beam lives on, so that the dictionary is usable, and
        writable to file, without a GPU.
        ...
        # `copy_as_numpy` returns a host copy whether the array lives on the
        # GPU or not, so no branching on the backend is needed here.
        arrays = {
            "dt": self._dt.copy_as_numpy(),
            "dE": self._dE.copy_as_numpy(),
            "flags": self._flags.copy_as_numpy(),
            "ids": self._ids.copy_as_numpy(),
        }

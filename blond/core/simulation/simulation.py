...
class Simulation(Preparable):
    ...
    def warmup(
        ...
        Runs ``n_turns`` of the normal per-turn element pipeline while
        holding the macroparticle coordinates fixed (restored after every
        warmup turn) and the ring turn/section position pinned at its
        starting value (so the RF/energy program never advances), so that
        turn-dependent internal state in wakefield solvers (e.g. deques of
        past profiles, pole-residue decay states, multi-turn induced-voltage
        buffers) and RF/beam feedbacks (IIR filters, delay lines) reaches
        equilibrium with the beam's current bunch shape before real
        multi-turn tracking begins. After the first warmup turn, all
        profiles (whether attached to a ``WakeField`` or standalone ring
        elements) are computed once and held static for the remaining
        turns.

        Useful at injection, where the collective-effect state should
        already be equilibrated at turn 0 instead of drifting there over
        hundreds of real turns.

        Parameters
        ----------
        beam
            The beam to warm up. Its macroparticle coordinates are restored
            to their pre-warmup values when this method returns.
        n_turns
        ...
        run_simulation : Execute the real beam dynamics tracking.

        Notes
        -----
        - This method does not advance ``self.turn_counter``,
          ``self.section_counter``, or ``beam.reference`` — every warmup
          turn is executed at the turn index (and RF/energy program point)
          that was current when ``warmup()`` was called, and
          ``beam.reference.time``/``beam.reference.total_energy`` are left
          to advance naturally turn-to-turn during warmup (required for
          correct multi-turn wakefield solver bookkeeping), then reset to
          their pre-warmup values once, when this method returns.
        - The beam's macroparticle coordinates (``dt``, ``dE``, ``flags``,
          ``ids``, ``intensity``) are unchanged after this call, as is
          every profile's ``active`` flag.
        - Only wakefield solver / feedback internal state is intentionally
          left mutated by this call — that is the point of the method.
        ...
        self.finalize(beams=(beam,), n_turns=n_turns)

        shape_snapshot = beam.snapshot_state()
        start_time = beam.reference.time
        start_total_energy = beam.reference.total_energy
        start_turn = self.turn_counter.value
        start_section = self.section_counter.value

        # Gate on `profile.active`, not `wakefield.track_profile`: a
        # WakeField's profile may also be independently registered as its
        # own ring element (tracked on its own, outside the WakeField's
        # `_track`), and only `profile.active` is checked by both paths.
        # `wakefield.update_induced_voltage` is deliberately left untouched
        # — the solver state must keep updating every warmup turn. Profiles
        # not attached to any WakeField are frozen too, for consistency
        # with "the bunch shape is fixed for the whole warmup".
        wakefields = self.ring.elements.get_elements(WakeField, recursive=True)
        standalone_profiles = self.ring.elements.get_elements(
            ProfileBaseClass, recursive=True
        )
        profiles = {wakefield.profile for wakefield in wakefields} | set(
            standalone_profiles
        )
        original_profile_active = {
            profile: profile.active for profile in profiles
        }
        ...
        try:
            for i in iterator:
                self.turn_counter.value = start_turn
                self.section_counter.value = start_section
                ...
                beam.restore_state(shape_snapshot)

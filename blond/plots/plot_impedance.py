...
def plot_impedance_vs_frequency(
    ...
    figure_index: int = 0,
    plot_total_impedance: bool = True,
    plot_spectrum: bool = False,
    plot_interpolated_impedances: bool = False,
    style: str = "-",
    cut_left_right: Optional[tuple[float, float]] = None,
    cut_up_down: Optional[tuple[float, float]] = None,
    ...
    if plot_total_impedance:
        ...
        ax1.plot(
            induced_voltage_freq.freq,
            (
                induced_voltage_freq.total_impedance.real
                * induced_voltage_freq.profile.bin_size
            ),
            style,
        ...
        ax1.plot(
            induced_voltage_freq.freq,
            (
                induced_voltage_freq.total_impedance.imag
                * induced_voltage_freq.profile.bin_size
            ),
            style,
        ...
        if plot_spectrum:
            ax2 = ax1.twinx()
            ax2.plot(
                induced_voltage_freq.profile.beam_spectrum_freq,
                np.abs(induced_voltage_freq.profile.beam_spectrum),
                label="Beam Spectrum",
            )
        ...
        for i in range(len(induced_voltage_freq.impedance_source_list)):
            if (
                isinstance(
                    induced_voltage_freq.impedance_source_list[i], InputTable
                )
                and not plot_interpolated_impedances
            ):
                ax0.plot(
                    induced_voltage_freq.impedance_source_list[
                        i
        ...
        fign1 = (
            dirname + "/real_imp_vs_" + fig_suffix + "_"
            "%d" % figure_index + ".png"
        )
        if plot_spectrum:
            ax2 = ax0.twinx()
        ...
        if show_plots:
            ...
            plt.clf()
        fign2 = (
            dirname + "/imag_imp_vs_" + fig_suffix + "_"
            "%d" % figure_index + ".png"
        )
...
def plot_wake_vs_time(
    induced_voltage_time: InducedVoltageTime,
    figure_index: int = 0,
    plot_total_wake: bool = True,
    plot_interpolated_wake: bool = False,
    style: str = "-",
    cut_left_right: Optional[tuple[float, float]] = None,
    cut_up_down: Optional[tuple[float, float]] = None,
    dirname: str | PathLike[str] = "fig",
    ...
    if plot_total_wake:
        ...
        ax1.plot(
            induced_voltage_time.time,
            induced_voltage_time.total_wake,
            style,
            label="Wake",
        )
        ...
        for i in range(len(induced_voltage_time.wake_source_list)):
            if (
                isinstance(
                    induced_voltage_time.wake_source_list[i], InputTable
                )
                and not plot_interpolated_wake
            ):
                ax0.plot(
                    induced_voltage_time.wake_source_list[i].time_array,
                ...
                ax0.plot(
                    induced_voltage_time.wake_source_list[i].new_time_array,
                    induced_voltage_time.wake_source_list[i].wake,
                    style,
                )
                ax0.set_xlim(cut_left_right)
                ax0.set_ylim(cut_up_down)
...
def plot_induced_voltage_vs_bin_centers(
    ...
    ax0 = plt.axes()
    plt.plot(
        total_induced_voltage.profile.bin_centers,

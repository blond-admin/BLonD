...
void where_more_than(const real_t *__restrict__ data, const int n,
                     const real_t c1, bool *__restrict__ res) {
...
void add_int_vector(const int *__restrict__ a, const int *__restrict__ b,
                    const int size, int *__restrict__ result) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    result[i] = a[i] + b[i];
  }
...
void add_uint16_vector(const uint16_t *__restrict__ a,
                       const uint16_t *__restrict__ b, const int size,
...
void add_uint32_vector(const uint32_t *__restrict__ a,
                       const uint32_t *__restrict__ b, const int size,
...
void add_longint_vector(const long *__restrict__ a, const long *__restrict__ b,
                        const int size, long *__restrict__ result) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    result[i] = a[i] + b[i];
  }
...
void add_double_vector(const double *__restrict__ a,
                       const double *__restrict__ b, const int size,
...
void add_float_vector(const float *__restrict__ a, const float *__restrict__ b,
                      const int size, float *__restrict__ result) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    result[i] = a[i] + b[i];
  }
...
void add_int_vector_inplace(int *__restrict__ a, const int *__restrict__ b,
                            const int size) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    a[i] = a[i] + b[i];
...
void add_longint_vector_inplace(long *__restrict__ a,
                                const long *__restrict__ b, const int size) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    a[i] = a[i] + b[i];
  }
...
void add_float_vector_inplace(float *__restrict__ a,
                              const float *__restrict__ b, const int size) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i) {
    a[i] = a[i] + b[i];
  }
...
void convolution(const real_t *__restrict__ signal, const int SignalLen,
                 const real_t *__restrict__ kernel, const int KernelLen,
...
real_t stdev(const real_t *__restrict__ data, const int n) {
...
void fast_sinv(const real_t *__restrict__ in, const int size,
...
void fast_cosv(const real_t *__restrict__ in, const int size,
               real_t *__restrict__ out) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i)
    out[i] = FAST_COS(in[i]);
...
void fast_expv(const real_t *__restrict__ in, const int size,
               real_t *__restrict__ out) {
#pragma omp parallel for
  for (int i = 0; i < size; ++i)
    out[i] = FAST_EXP(in[i]);
...
*/
void interp(const real_t *__restrict__ x, const int N,
...
@y: the interpolated values, same shape as x
*/
void interp_const_space(const real_t *__restrict__ x, const int N,
                        const real_t *__restrict__ xp, const int M,
                        const real_t *__restrict__ yp, const real_t left,
                        const real_t right, real_t *__restrict__ y) {
  ...
  const real_t c =
      (x[0] - xp[0] + (1 - offset) * (xp[1] - xp[0])) / (xp[1] - xp[0]);
...
void interp_const_bin(const real_t *__restrict__ x, const int N,
  ...
  /**
   * @brief Linear interpolation with constant bin size (space between xp values
   * is constant)
   *
   * @x: x-coordinates of the interpolated values
   * @N: The x array size
...
void cumtrapz_wo_initial(const real_t *__restrict__ f, const real_t deltaX,
                         const int nsub, real_t *__restrict__ psum) {
...
// Function to implement integration of f(x) over the interval
// [a,b] using the trapezoid rule with nsub subdivisions.
void cumtrapz_w_initial(const real_t *__restrict__ f, const real_t deltaX,
                        const real_t initial, const int nsub,
...
real_t trapz_var_delta(const real_t *__restrict__ f,
                       const real_t *__restrict__ deltaX, const int nsub) {
  // initialize the partial sum to be f(a)+f(b) and
  // deltaX to be the step size using nsub subdivisions
...
void arange_double(const double start, const double stop, const double step,
...
void arange_float(const float start, const float stop, const float step,
...
void arange_int(const int start, const int stop, const int step,
...
void scalar_mul_int32(const int *__restrict__ a, const int b, const int n,
...
void scalar_mul_int64(const long *__restrict__ a, const long b, const int n,
                      long *__restrict__ res) {
#pragma omp parallel for
  for (int i = 0; i < n; ++i) {
    res[i] = a[i] * b;
  }
...
void scalar_mul_float32(const float *__restrict__ a, const float b, const int n,
...
void scalar_mul_complex64(const std::complex<float> *__restrict__ a,
                          const std::complex<float> b, const int n,
                          std::complex<float> *__restrict__ res) {
#pragma omp parallel for
  for (int i = 0; i < n; ++i) {
    res[i] = a[i] * b;
  }
...
void scalar_mul_complex128(const std::complex<double> *__restrict__ a,
                           const std::complex<double> b, const int n,
                           std::complex<double> *__restrict__ res) {
#pragma omp parallel for
  for (int i = 0; i < n; ++i) {
    res[i] = a[i] * b;
  }
...
void vector_mul_float32(const float *__restrict__ a,
...
void vector_mul_float64(const double *__restrict__ a,

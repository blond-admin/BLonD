...
slices_ring = Slices(rf_params, my_beam, 200, cut_left = 0.0, cut_right = 2.0*np.pi, cuts_unit = 'rad')

#Phase loop
configuration = {'machine': 'PSB', 'PL_gain': 1./25.e-6*np.ones(n_turns+1), 'RL_gain': [0,0], 'PL_period': 10.e-6, 'RL_period': 7}
...
plots = Plot(general_params, rf_params, my_beam, 50, n_turns, 0.0, 2*np.pi, -1e6, 1e6, xunit= 'rad',
             separatrix_plot= True, Slices = slices_ring, format_options = format_options,

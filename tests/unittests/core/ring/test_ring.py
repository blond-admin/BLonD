...
class TestRing(unittest.TestCase):
    ...
    def test_force_section_index_compatibility(self):
        ...
        assert (
            element_forced._section_index
            == self.ring.elements.elements[location].section_index
        )
        ...
        assert (
            element_forced._section_index
            == self.ring.elements.elements[
                len(self.ring.elements.elements) - 1
            ].section_index
        )
    ...
    def test_non_mandatory_element_checking_drifts(self):
        ...
        sim = Simulation(
            ring, ConstantMagneticCycle(value=1e12, reference_particle=lead_82)
        )
        beam = Beam(
            intensity=1,
    ...
    def test_assert_radiation_integrals(self):
        radiation_integrals = np.array(
            [
                0.646747216157,
                0.0005936549319,
        ...
        SR_ring = Ring(10.0, radiation_integrals=radiation_integrals)
        SR_ring.add_drifts(n_drifts_per_section=2, n_sections=1)
        SR_ring.assert_radiation_integrals()

        SR_ring.elements.elements[0]._radiation_integrals = radiation_integrals
        SR_ring.assert_radiation_integrals()

...
class TestBeamPreparationMuCol(unittest.TestCase):
    ...
    def test_copy_beam_data_from_other_beam(self):
        ...
        filename = "testfile.npz"
        beam.dt = np.linspace(-50e-9, 50e-9, num=100)
        beam.dE = np.linspace(-50e9, 50e9, num=100)
        beam._flags = np.ones_like(beam.dE)
        beam._ids = np.arange(len(beam.dE))

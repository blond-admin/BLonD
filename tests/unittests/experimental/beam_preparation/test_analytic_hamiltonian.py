...
def _single_bucket_well(n_time=400, amplitude=53.6):
    ...
    well = (
        amplitude
        * backend.cos(
            np.pi * backend.arange(n_time, dtype=backend.float) / (n_time - 1)
        )
        ** 2
    )
    return time, well
...
def test_grid_shape_and_convention():
    ...
    assert backend.all(backend.isclose(time_grid[0, :], time))
    assert backend.all(backend.isclose(time_grid[:, 0], time[0]))
...
def test_hamiltonian_formula():
    ...
    assert backend.all(backend.isclose(hamilton, expected))
...
def test_default_deltaE_frame_is_separatrix():
    ...
    assert backend.isclose(deltaE_grid.max(), deltaE_max)
    assert backend.isclose(deltaE_grid.min(), -deltaE_max)
    ...
    assert backend.isclose(hamilton[:, i_min].max(), well.max(), rtol=1e-9)
...
def test_min_of_hamiltonian_is_zero_at_center():
    ...
    assert backend.isclose(hamilton.min(), 0.0)
...
def test_custom_energy_range():
    ...
    assert backend.isclose(deltaE_grid.min(), -1e8)
    assert backend.isclose(deltaE_grid.max(), 1e8)

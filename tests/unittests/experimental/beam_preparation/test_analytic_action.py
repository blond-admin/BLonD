...
def test_action_matches_harmonic_oscillator():
    ...
    assert backend.all(
        backend.isclose(sorted_j[sel], expected[sel], rtol=5e-4)
    )
...
def test_separatrix_action_matches_bucket_area():
    ...
    assert backend.isclose(sorted_j[-1], expected_action, rtol=1e-6)
...
def test_below_transition_separatrix_action():
    ...
    assert not backend.any(backend.isnan(sorted_j))
...
def test_action_on_nonuniform_grid():
    ...
    expected_action = 4.0 * deltaE_max / (np.pi * OMEGA_RF)
    assert backend.isclose(sorted_j[-1], expected_action, rtol=1e-5)
...
def test_emittance_beyond_bucket_raises():
    ...
    bucket_area = 2.0 * np.pi * float(sorted_j[-1])
    with pytest.raises(ValueError):
...
def test_action_grid_shape_and_outside_bucket():
    ...
    outside = action_grid(
        backend.array([sorted_h[-1] * 10], dtype=backend.float),
        sorted_h,
        sorted_j,
    )
    assert float(outside[0]) == np.inf
    # Inside, the action stays bounded and non-negative.
    inside = hamilton <= sorted_h[-1]
    assert backend.all(backend.isfinite(action_2D[inside]))
    assert backend.all(action_2D[inside] >= 0.0)
...
def test_split_well_via_loosened_tolerance_matches_legacy_route():
    ...
    assert max_backstep < 1e-4 * float(sorted_j[-1])
    ...
    assert backend.isclose(sorted_j_2[-1], sorted_j[-1], rtol=1e-9)

...
class TestResonatorAboveProfileCutoff(unittest.TestCase):
    ...
    Bin-averaging the wake over the source bin alone (a box, ``sinc(f dt)``
    with NumPy's ``sinc(x) = sin(pi x) / (pi x)``) does not rescue this:
    sampling folds the resonance from above the Nyquist frequency down onto
    the impedance's inductive low-frequency flank -- the only part of the
    impedance this bunch samples -- and a single box suppresses the alias
    images only as ``1 / |j|``, so it very nearly cancels the flank.
    Averaging over the observation bin as well (``sinc(f dt)**2``) takes
    them to ``1 / j**2`` and restores the amplitude; the third box the
    kernel actually uses (``sinc(f dt)**3``) takes them one order further,
    which is what stops the tail from leaking the resonance's *resistive*
    part into this reactive band -- see `Resonators._wake_bin_average` and
    `TestNoHalfBinLag`.
    """
...
class TestNoHalfBinLag(unittest.TestCase):
    """
    The solvers must not leak resistive impedance into a reactive band.

    Peak-amplitude comparisons are blind to what a two-box bin-average
    leaves behind. Sampling the wake every ``dt`` folds the impedance, and
    two boxes suppress the alias images only as ``1 / j**2``; for a
    resonance far above Nyquist that tail brings the resonator's
    *resistive* part down onto frequencies where the true impedance is
    essentially reactive. The third box in `Resonators._wake_bin_average`
    takes the tail to ``1 / |j|**3`` and the leak with it. This is a
    bandwidth argument, not a timing one: ``sinc(f dt)**k`` is real below
    Nyquist and carries no phase.

    The leak shows up two ways, and the class name records the one it was
    first read as. A reactive impedance does no net work on the beam, so a
    fabricated resistive component is directly measurable as loss factor --
    14-fold too large at the coarsest binning here. It also moves the
    voltage's zero crossing, which is what made it look like a delay.
    """
    ...
    def test_every_solver_is_on_time(self):
        ...
        The alias leak of the two-box average surfaced as an apparent
        delay: the time-domain solvers sat 0.47 bins late at
        ``f_cutoff = f_res / 10`` and 0.39 bins late at ``f_res / 3``, while
        the frequency-domain solver, which never samples the wake, was
        always on time. All three must now agree with the continuum to well
        inside a tenth of a bin.
        """
    ...
    def test_reactive_impedance_does_no_spurious_work(self):
        """
        Leaked resistive impedance is measurable as spurious energy loss.

        The bunch only samples the resonator's inductive flank, so the loss
        factor ``sum(V * lambda)`` is small and anything resistive folded
        into that band inflates it. The two-box average made it 14x too
        large at ``f_cutoff = f_res / 10``; the time-domain solvers must now
        stay within a few percent of the frequency-domain reference, which
        never samples the wake and so has no alias tail at any binning.
        """
        for ratio in (0.1, 0.3, 1.0):
            _maybe_draw(ratio)
            profile, voltages = _induced_voltages(ratio * F_RES)

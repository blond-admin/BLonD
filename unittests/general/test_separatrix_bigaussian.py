...
class TestSeparatrixBigaussian(unittest.TestCase):
    ...
    def setUp(self, negativeEta=True, acceleration=True, singleRF=True):
        ...
        tau_0 = 50.0e-9  # Initial bunch length, 4 sigma [s]

        # Machine parameters
        C = 1567.5  # Machine circumference [m]
        p_1i = 3.0e9  # Synchronous momentum [eV/c]
        p_1f = 30.0e9  # Synchronous momentum, final
        p_2f = 40.0e9  # Synchronous momentum [eV/c]
        p_2i = 60.0e9  # Synchronous momentum, final
        gamma_t = 31.6  # Transition gamma
        alpha_1 = -1.0 / gamma_t / gamma_t  # First order mom. comp. factor
        alpha_2 = 1.0 / gamma_t / gamma_t  # First order mom. comp. factor

        # RF parameters
        h = [9, 18]  # Harmonic number
        V = [1800.0e3, 110.0e3]  # RF voltage [V]
        phi_1 = [np.pi + 1.0, np.pi / 6 + 2.0]  # Phase modulation/offset
        phi_2 = [1.0, np.pi / 6 + 2.0]  # Phase modulation/offset
        N_t = 43857
        ...
        if negativeEta:
            if acceleration:
                # eta < 0, acceleration
                general_params = Ring(
                    C, alpha_1, np.linspace(p_1i, p_1f, N_t + 1), Proton(), N_t
                )
            elif not acceleration:
                # eta < 0, deceleration
                general_params = Ring(
                    C, alpha_1, np.linspace(p_1f, p_1i, N_t + 1), Proton(), N_t
                )

            if singleRF:
                rf_params = RFStation(
                    general_params, 9, 1.8e6, np.pi + 1.0, n_rf=1
                )
            elif not singleRF:
            ...
            if acceleration:
                # eta > 0, acceleration
                general_params = Ring(
                    C, alpha_2, np.linspace(p_2i, p_2f, N_t + 1), Proton(), N_t
                )
            elif not acceleration:
                # eta > 0, deceleration
                general_params = Ring(
                    C, alpha_2, np.linspace(p_2f, p_2i, N_t + 1), Proton(), N_t
                )

            if singleRF:
                rf_params = RFStation(general_params, 9, 1.8e6, 1.0, n_rf=1)
            elif not singleRF:
        ...
        slices = Profile(
            beam,
            CutOptions(cut_left=0.0e-9, cut_right=600.0e-9, n_slices=1000),
        )
        slices.track()
        configuration = {
            "machine": "LHC",
        ...
        self.dE_sep = separatrix(
            general_params,
            rf_params,
            [-5.0e-7, -3.0e-7, 1.0e-7, 3.0e-7, 7.0e-7, 9.0e-7],
        )

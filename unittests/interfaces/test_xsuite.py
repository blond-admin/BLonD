...
try:
    import xpart as xp
    import xtrack as xt
    from blond.legacy.blond2.interfaces.xsuite import (
        BlondElement,
        BlondObserver,
        EnergyUpdate,
...
class TestXsuiteBLonDTransforms(unittest.TestCase):
    ...
    def testBlondToXsuiteTransform(self):
        ...
        dts = np.linspace(
            self.rfstation.t_rf[0, 0] * 1e-3, self.rfstation.t_rf[0, 0], 10
        )
        ...
        zeta, ptau = blond_to_xsuite_transform(
            ...
            dEs,
            self.ring.beta[0, 0],
            self.ring.energy[0, 0],
            self.rfstation.omega_rf[0, 0],
            self.rfstation.phi_s[0],
        )
    ...
    def testXsuiteToBlondTransform(self):
        ...
        zeta = np.linspace(-0.37, 0.37, 10)
        ptau = np.linspace(-4e-4, 4e-4, 10)
    ...
    def testTransformInverse(self):
        # Arrays of time and energy offsets
        dts = np.linspace(
            self.rfstation.t_rf[0, 0] * 1e-3, self.rfstation.t_rf[0, 0], 20
        )
...
class TestXsuiteLHC(unittest.TestCase):
    def setUp(self):
        ...
        self.alpha = (
            0.00034849575112251314  # First order mom. comp. factor [-]
        )
    ...
    def testSingleParticle(self):
        ...
        dt_inter, dE_inter = self.performSingleParticleInterfaceSimulation(
            line, blond_tracker, beam
        )
        ...
        blond_tracker, beam = self.singleParticleBLonDSimulation()
        dt_blond, dE_blond = self.performSingleParticleBLonDSimulation(
            blond_tracker, beam
        )
    ...
    def testSingleParticleRamp(self):
        ...
        line.insert_element(
            index="blond_cavity", element=energy_update, name="energy_update"
        )
    ...
    def testSingleParticleTwoRFStations(self):
        ...
        blond_tracker_1, blond_tracker_2, beam = (
            self.singleParticleBLonDSimulation(two_rfstations=True)
        )
        ...
        # Insert the BLonD elements
        line.insert_element(
            index="matrix", element=cavity_1, name="blond_cavity_1"
        )
        line.insert_element(
            index="matrix_2", element=cavity_2, name="blond_cavity_2"
        )

        dt_inter, dE_inter = self.performSingleParticleInterfaceSimulation(
            line, blond_tracker_1, beam
        )
        dt_inter = dt_inter / np.max(dt_inter)
        dE_inter = dE_inter / np.max(dE_inter)
        ...
        dt_blond, dE_blond = self.performSingleParticleBLonDSimulation(
            blond_tracker_1, beam, blond_tracker_2
        )
        dt_blond = dt_blond / np.max(dt_blond)
        dE_blond = dE_blond / np.max(dE_blond)
    ...
    def singleParticleBLonDSimulation(
        self,
        ramp: bool = False,
        two_rfstations: bool = False,
        ...
        if ramp:
            ...
            if pure_blond:
                cycle = np.concatenate(
                    (
                        np.array([self.p_s]),
                        np.linspace(self.p_s, self.p_f, N_t + 1)[:-1],
                    )
                )
            ...
            if two_rfstations:
                ...
                ring = Ring(
                    self.C, self.alpha, self.p_s, Proton(), n_turns=N_t
                )
        ...
        if two_rfstations:
            ...
            return (
                RingAndRFTracker(rfstation, beam),
    ...
    def performSingleParticleInterfaceSimulation(
        self, line, blond_track, beam
    ):
        r"""
        ...
        line.particle_ref = xp.Particles(
            p0c=self.p_s, mass0=xp.PROTON_MASS_EV, q0=1.0
        )
        line.build_tracker()
        ...
        for i in range(N_t):
            dt_array[i], dE_array[i] = xsuite_to_blond_transform(
                ...
                - blond_track.rf_params.phi_rf[0, 0],
    ...
    @staticmethod
    def performSingleParticleBLonDSimulation(
        blond_track, beam, second_blond_tracker: RingAndRFTracker = None
    ):
        r"""
...
class TestXsuitePSB(unittest.TestCase):
    def setUp(self):
        ...
        )  # Machine circumference, radius 25 [m]
    ...
    def singleParticleBLonDSimulation(
        self, ramp: bool = False, pure_blond: bool = False
    ):
        r"""
        Method to generate a BLonD simulation in the LHC with a single particle with a time offset with respect to the
        RF bucket.

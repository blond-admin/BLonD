...
class TestGeneralParameters(unittest.TestCase):
    ...
    def test_synchronous_data_exception(self):
        ...
        with self.assertRaisesRegex(
                RuntimeError, 'ERROR in Ring: sychronous data does not ' +
                'match the time data',
    ...
    def test_momentum_shape_exception(self):
        ...
        with self.assertRaisesRegex(
                RuntimeError, 'ERROR in Ring: Number of sections and ' +
                'momentum data do not match!',
                msg='No RuntimeError for wrong shape of momentum!'):

            Ring(self.n_turns, self.C, self.alpha, momentum, self.particle,
                 n_stations=self.num_sections)

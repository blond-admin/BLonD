...
from blond.legacy.blond2.beam.profile import CutOptions, Profile, FitOptions
...
class TestInducedVoltageFreq(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.induced_voltage_freq = InducedVoltageFreq(
            Beam=None,
            Profile=None,
            impedance_source_list=None,
            frequency_resolution=None,
            multi_turn_wake=None,
            front_wake_length=None,
            RFParams=None,
            mtw_mode=None,
            use_regular_fft=None,
        )
...
class TestInducedVoltageResonator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.induced_voltage_resonator = InducedVoltageResonator(
            Beam=None, Profile=None, Resonators=None, timeArray=None
        )
    ...
    def setUp(self):
        # TODO: implement test for `__init__`
        self.induced_voltage_time = InducedVoltageTime(
            Beam=None,
            Profile=None,
...
class TestTotalInducedVoltage(unittest.TestCase):
    ...
    def setUp(self):
        # TODO: implement test for `__init__`
        self.total_induced_voltage = TotalInducedVoltage(
            Beam=None, Profile=None, induced_voltage_list=None
        )
...
class Test_InducedVoltage(unittest.TestCase):
    ...
    def setUp(self):
        # TODO: implement test for `__init__`
        self.__induced_voltage = _InducedVoltage(
            Beam=None,
            Profile=None,
            frequency_resolution=None,
            wake_length=None,
...
class TestInducedVoltageResonatorMultiTurnWake(unittest.TestCase):
    def setUp(self):
        ...
        energy_program = np.array(
            [1 * 25e9, 1 * 25e9, 1.04 * 25e9, 1.05 * 25e9]
        )

        self.ring = Ring(
            ring_length=sectionlengths,
            alpha_0=alpha_c,
            particle=Proton(),
            n_turns=self.n_turns,
            n_sections=self.n_stations,
            synchronous_data=energy_program,
        )
        ...
        self.n_slices = 2**9
        self.beam = Beam(
            ring=self.ring, n_macroparticles=1001, intensity=int(1e10)
        )
        ...
        self.profile = Profile(
            self.beam, cut_options, FitOptions(fit_option="gaussian")
        )

        self.resonator = Resonators(
            self.R_shunt, self.f_res + fdet, self.Q_factor
        )

    def test_total_induced_voltage(self):
        ivr = InducedVoltageResonator(
            beam=self.beam,
        ...
        totalvolt = TotalInducedVoltage(
            beam=self.beam, profile=self.profile, induced_voltage_list=[ivr]
        )
        ...
        for _ in range(self.n_turns):
            ...
            induced_voltages.append(
                longitudinal_tracker.totalInducedVoltage.induced_voltage
            )

        induced_voltages = np.array(induced_voltages)

        folder = os.path.abspath(os.path.dirname(__file__)) + "/data/"
        expected_voltages = np.load(
            folder + "mtw_resonator_induced_voltage.npy"
        )
        np.testing.assert_allclose(
            induced_voltages, expected_voltages, rtol=1e-5, atol=1e-1
        )

...
# Tests tolerance
absolute_tolerance = 0.0  # 0. / 1e-16
relative_tolerance = 1e-9  # 1e-9 / 0.


def linear_drift(original_distribution_dE, eta_0, beta, energy, trev):
    drift_kick = trev * eta_0 * original_distribution_dE / (beta**2.0 * energy)

    return drift_kick


def legacy_drift(
    original_distribution_dE, eta_0, eta_1, eta_2, beta, energy, trev
):
    drift = trev * (
        1.0
        / (
            1.0
            - eta_0 * original_distribution_dE / (beta**2.0 * energy)
            - eta_1 * (original_distribution_dE / (beta**2.0 * energy)) ** 2.0
            - eta_2 * (original_distribution_dE / (beta**2.0 * energy)) ** 3.0
        )
        - 1.0
    )
...
def exact_drift(
    original_distribution_dE, alpha_0, alpha_1, alpha_2, beta, energy, trev
):
    delta = (
        np.sqrt(
            1.0
            ...
            / beta**2.0
            * (
                (original_distribution_dE / energy) ** 2.0
                + 2.0 * (original_distribution_dE / energy)
            )
        )
        - 1.0
    )
    ...
    drift = trev * (
        (1.0 + alpha_0 * delta + alpha_1 * delta**2.0 + alpha_2 * delta**3.0)
        * (1.0 + (original_distribution_dE / energy))
        / (1.0 + delta)
        - 1.0
    )
...
def expected_drift(
    ...
    momentum = np.sqrt(energy**2.0 - mass_energy**2.0)
    particle_momentum = np.sqrt(particle_total_energy**2.0 - mass_energy**2.0)
    ...
    particle_circ = circumference * (
        1.0
        + alpha_0 * particle_dp / momentum
        + alpha_1 * (particle_dp / momentum) ** 2.0
        + alpha_2 * (particle_dp / momentum) ** 3.0
    )
...
class TestDrift(unittest.TestCase):
    ...
    C = 2 * np.pi * 25.0  # Machine circumference [m]
    Ek = 160e6  # Kinetic energy [eV]
    gamma_t = 4.1  # Transition gamma
    alpha_0 = 1.0 / gamma_t / gamma_t  # First order mom. comp. factor
    alpha_1 = 10 * alpha_0
    alpha_2 = 100 * alpha_0
    ...
    def test_exact_order0(self):
        ...
        original_distribution_dt += self.ring.n_turns * exact_drift(
            ...
            self.ring.alpha_1[0, 0],
            self.ring.alpha_2[0, 0],
            self.ring.beta[0, 0],
            self.ring.energy[0, 0],
            self.ring.t_rev[0],
        )

...
def orig_rf_volt_comp(tracker):
    ...
    for rf_system in range(tracker.rf_params.n_rf):
        voltages = np.append(
            voltages, tracker.rf_params.voltage[rf_system, tracker.counter[0]]
        )
        omega_rf = np.append(
            omega_rf, tracker.rf_params.omega_rf[rf_system, tracker.counter[0]]
        )
        phi_rf = np.append(
            phi_rf, tracker.rf_params.phi_rf[rf_system, tracker.counter[0]]
        )
    ...
    if tracker.cavityFB:
        if tracker.cavityFB[0]:
            rf_voltage = voltages[0, 0] * tracker.cavityFB[0].V_corr * np.sin(
                omega_rf[0, 0] * tracker.profile.bin_centers
                + phi_rf[0, 0]
                + tracker.cavityFB[0].phi_corr
            ) + np.sum(
                voltages.T[1:]
                * np.sin(
                    omega_rf.T[1:] * tracker.profile.bin_centers + phi_rf.T[1:]
                ),
                axis=0,
            )
    else:
        rf_voltage = np.sum(
            voltages.T
            * np.sin(omega_rf.T * tracker.profile.bin_centers + phi_rf.T),
            axis=0,
        )
...
class Test:
    ...
    def _setUp(self):
        ...
        self.ring = Ring(
            26658.883,
            1.0 / 55.759505**2,
            np.linspace(450e9, 460.005e9, self.n_turns + 1),
            Proton(),
            self.n_turns,
        )
        ...
        self.rf_station = RFStation(
            self.ring,
            [35640],
            6e6 * np.linspace(1, 1.1, self.n_turns + 1),
            [0],
        )
        dt_test = np.linspace(
        ...
        self.profile = Profile(
            self.beam,
            CutOptions(
                n_slices=100, cut_left=0, cut_right=self.rf_station.t_rf[0, 0]
            ),
            FitOptions(fit_option="gaussian"),
    ...
    def test_executes_gpu(self):
        ...
        self._setUp()
        dt_org = deepcopy(self.long_tracker.beam.dt)
        self.long_tracker._kickdrift_considering_periodicity_gpu(
            turn=self.turn
        )
        dt_new = deepcopy(self.long_tracker.beam.dt)
        # assert that _fix_periodicity is acting on dt
    ...
    def test_gpu_faster(self):
        ...
        t0 = time.perf_counter()
        for i in range(100):
            self.long_tracker._kickdrift_considering_periodicity(
                turn=self.turn
            )
        Device().synchronize()
        t1 = time.perf_counter()
...
class TestFullRingAndRF(unittest.TestCase):
    ...
    def test_potential_well_generation(self):
        ...
        self.full_ring_and_rf.potential_well_generation(
            turn=None,
            n_points=None,
...
class TestRingAndRFTracker(unittest.TestCase):
    ...
    def setUp(self):
        # TODO: implement test for `__init__`
        self.ring_and_rf_tracker = RingAndRFTracker(
            RFStation=None,
            Beam=None,
            solver=None,
            BeamFeedback=None,

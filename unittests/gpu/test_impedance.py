...
class TestImpedanceBigaussianData:
    ...
    def setup_method(self):
        ...
        bigaussian(
            self.general_params, self.rf, self.beam, self.tau_0 / 4, seed=1
        )
        ...
        table = np.loadtxt(
            os.path.join(
                this_directory,
                "../../__EXAMPLES/input_files/EX_05_new_HQ_table.dat",
            ),
            comments="!",
        )
    ...
    def test_ind_volt(self, number_slices, mode):
        ...
        cut_options_gpu = CutOptions(
            cut_left=0,
            cut_right=2 * np.pi,
            n_slices=number_slices,
            rf_station=self.rf_gpu,
            cuts_unit="rad",
        )
        ...
        if mode == "time":
            ...
            cp.testing.assert_allclose(
                ind_volt_gpu.total_impedance,
                ind_volt.total_impedance,
                rtol=1e-8,
                atol=0,
            ...
            ind_volt_gpu = InducedVoltageResonator(
                self.beam_gpu, slice_beam_gpu, self.resonator
            )
        ...
        cp.testing.assert_allclose(
            slice_beam_gpu.n_macroparticles,
        ...
        cp.testing.assert_allclose(
            tot_volt_gpu.induced_voltage,
            tot_volt.induced_voltage,
            rtol=1e-8,
            atol=1e-6,
        )

    @pytest.mark.parametrize(
        "number_slices,mode,n_iter",
        [
            (256, "time", 1),
            (1024, "time", 100),
            (256, "frequency", 1),
...
class TestImpedanceMTW:
    ...
    def setup_method(self):
        ...
        bigaussian(
            self.general_params, self.rf, self.beam, self.sigma_dt, seed=1
        )
    ...
    @pytest.mark.parametrize(
        "number_slices,mode,mtw_mode,n_iter",
        [
            (256, "time", "time", 1),
            (1024, "time", "time", 100),
            #   (256, 'time', 'freq', 1), (1024, 'time', 'freq', 100),
            (256, "frequency", "time", 1),
            (1024, "frequency", "time", 100),

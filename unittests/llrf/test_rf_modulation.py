...
class TestRFModulation(unittest.TestCase):
    def test_construct(self):
        ...
        stringMsg = (
            "Wrong shape input should raise an InputDataError exception"
        )
        with self.assertRaises(blond_exceptions.InputDataError, msg=stringMsg):
        ...
        with self.assertRaises(
            TypeError, msg="Non-boolean input should raise TypeError"
        ):
    ...
    def test_interpolation(self):
        ...
        self.assertTrue(
            all(modulator._interp_param([[0, 1], [0, 1]]) == timebase),
            msg="Function interpolation incorrect",
        )

        self.assertTrue(
            all(modulator._interp_param(1) == 1),
            msg="Single valued interpolation incorrect",
        )

        with self.assertRaises(
            TypeError, msg="Error should be raised for wrong shape data"
        ):
    ...
    def test_modulation(self):
        ...
        modulator = rfMod.PhaseModulation(
            timebase,
            testFreqProg,
            testAmpProg,
            testOffsetProg,
        ...
        self.assertEqual(modulator.dphi[0], 0, msg="Start phase should be 0")

        self.assertEqual(
            modulator.dphi[-1], np.pi, msg="Start phase should be np.pi"
        )

        self.assertAlmostEqual(
            np.max(modulator.dphi),
            3.55908288285,
            5,
    ...
    def test_delta_omega(self):
        ...
        freqProg = np.array(
            [np.linspace(0, 1, 10000), np.linspace(1e6, 1e6, 10000)]
        )

        modulator = rfMod.PhaseModulation(timebase, 1, 1, 1, 1)

        modulator.dphi = (
            np.linspace(0, np.pi / 2, 250).tolist()
            + [np.pi / 2] * 500
            + np.linspace(np.pi / 2, 0, 250).tolist()
        )

        with self.assertRaises(
            blond_exceptions.InputDataError,
    ...
    def test_extender(self):
        ...
        harmonic = 8
        freqProg = np.array(
            [np.linspace(0, 1, 10000), np.linspace(1e6, 2e6, 10000)]
        )
        ...
        self.assertEqual(
            len(dPhi), 5, msg="dPhi Not correctly extended to n_rf"
        )

        self.assertEqual(
            len(dPhi), 5, msg="dOmega not correctly extended to n_rf"
        )

        for i in range(5):
            self.assertEqual(
                len(dPhi[i]), 2, msg="All dPhi members should have length 2"
            )
            self.assertEqual(
                len(dOmega[i]),
                2,

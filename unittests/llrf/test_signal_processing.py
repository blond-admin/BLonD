...
class TestRFCurrentSparse(unittest.TestCase):
    def setUp(self):
        ...
        bunch_spacing = 1 * self.rf.t_rf[0, 0]
        ...
        self.beam2 = Beam(
            ...
            total_number_of_bunches * number_of_bunches_per_batch * N_p,
        )
        ...
        for ba in range(number_of_batches):
            for i in range(number_of_bunches_per_batch):
                self.beam2.dt[
                    (i + ba * number_of_bunches_per_batch) * N_m : (
                        i + 1 + ba * number_of_bunches_per_batch
                    )
                    * N_m
        ...
        cut_options = CutOptions(
            cut_left=0,
            cut_right=(
                number_of_bunches_per_batch * bunch_spacing
                + batch_spacing * number_of_batches
            ),
            n_slices=int(
                (
                    number_of_bunches_per_batch * bunch_spacing
        ...
        for k in range(number_of_batches):
            batch_list[k * (int(batch_spacing / self.rf.t_rf[0, 0]) + 1)] = 1
        ...
        ax[0].plot(
            self.profile.bin_centers * 1e6,
            self.profile.n_macroparticles,
            label="Standard profile",
        )
        for profile_ind in self.sparse_profile.profiles_list:
            ax[0].plot(
    ...
    def test_downsampling(self):
        ...
        charges = (
            self.profile.beam.ratio
            * self.profile.beam.particle.charge
            * e
            * np.copy(self.profile.n_macroparticles)
        )
        charges_sparse = (
            self.sparse_profile.beam.ratio
            * self.sparse_profile.beam.particle.charge
            * e
        ...
        I_f_sparse = (
            2.0
            * charges_sparse
            * np.cos(self.omega * self.sparse_profile.bin_centers)
        )
        Q_f_sparse = (
            -2.0
            * charges_sparse
        ...
        self.assertEqual(
            self.beam_sparse.n_macroparticles, self.beam2.n_macroparticles
        )
        self.assertEqual(self.beam_sparse.intensity, self.beam2.intensity)
        self.assertAlmostEqual(
            tot_charges, 46000000000000.0, places=self.decimals
        )
...
class TestRFCurrent(unittest.TestCase):
    ...
    def test_1(self):
        ...
        self.profile.n_macroparticles = 2600 * np.exp(
    ...
    def test_3(self):
        ...
        rf_current = rf_beam_current(
            self.profile, self.omega, self.ring.t_rev[0], lpf=True
        )
    ...
    def test_4(self):
        ...
        N_b = 2.3e11
        bigaussian(
            self.ring, self.rf, self.beam, 0.1e-9, seed=1234, reinsertion=True
        )
        beam2 = Beam(self.ring, bunches * N_m, bunches * N_b)
        bunch_spacing = 5 * self.rf.t_rf[0, 0]
        buckets = 5 * bunches
        for i in range(bunches):
            beam2.dt[i * N_m : (i + 1) * N_m] = (
                self.beam.dt + i * bunch_spacing
            )
...
class TestLowPass(unittest.TestCase):
    def test_1(self):
        ...
        self.assertAlmostEqual(np.abs(y - xlow).max(), 0.0230316365, places=10)
...
class TestFeedforwardFilter(unittest.TestCase):
    ...
    def test_1(self):
        ...
        TWC.tau = 420e-9
        filter, n_taps, n_filling, n_fit = feedforward_filter(
            TWC, 4 / 125 * 1e-6, taps=31, opt_output=True
        )
        self.assertEqual(
            n_taps,
            31,
    ...
    def test_2(self):
        TWC = SPS3Section200MHzTWC()
        filter, n_taps, n_filling, n_fit = feedforward_filter(
            TWC, 1 / 40.0444e6, opt_output=True
        )
        self.assertEqual(
            n_taps,
            31,
    ...
    def test_3(self):
        ...
        self.assertEqual(
            n_fit, 62, msg="In TestFeedforwardFilter, test_3: n_fit incorrect"
        )
...
class TestFunctions(unittest.TestCase):
    ...
    def test_fir_filter_coefficients(self):
        ...
        fir_filter_coefficients(
            n_taps=None, sampling_freq=None, cutoff_freq=None
        )

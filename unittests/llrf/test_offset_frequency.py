...
class TestOffsetFrequency(unittest.TestCase):
    def setUp(self):
        ...
        self.ring = Ring(
            C, momentum_compaction, ([0, 25e-3], [3.13e8, 3.5e8]), Proton()
        )

        self.rf_params = RFStation(
            self.ring, [1, 2], [0, 0], [np.pi, np.pi * 0.95], 2
        )

    def test_fixed_freq(self):
        self.injection_frequency = offFreq.FixedFrequency(
            self.ring,
            self.rf_params,
        ...
        self.assertEqual(
            len(self.injection_frequency.frequency_prog),
            self.injection_frequency.end_transition_turn,
            msg="Fixed frequency + transition wrong length",
        )

        self.assertEqual(
            len(self.injection_frequency.phase_slippage[0]),
            self.injection_frequency.end_transition_turn,
        ...
        self.assertEqual(
            self.rf_params.omega_rf_d[0][0],
            self.injection_frequency.frequency_prog[
                self.injection_frequency.end_fixed_turn
            ],
            msg="Fixed frequency final value wrong",
        )

        self.assertAlmostEqual(
            self.rf_params.omega_rf_d[0][
                self.injection_frequency.end_transition_turn - 1
            ],
        ...
        self.assertEqual(
            self.injection_frequency.phase_slippage[0][0],
            0,
            msg="Phase slippage not starting at 0 for system 0",
        )

        self.assertEqual(
            self.injection_frequency.phase_slippage[1][0],
            0,
            msg="Phase slippage not starting at 0 for system 1",
        )

        self.assertSequenceEqual(
            self.injection_frequency.frequency_prog.tolist(),
            self.rf_params.omega_rf[0][
                : self.injection_frequency.end_transition_turn
            ].tolist(),
        ...
        self.assertAlmostEqual(
            self.rf_params.phi_rf[0][-1],
            -56.304959088,
            places=5,
            msg="System 1 end phase wrong",
        )
        self.assertAlmostEqual(
            self.rf_params.phi_rf[1][-1],
            -115.908590462,
            places=5,
            msg="System 1 end phase wrong",
        )
    ...
    def test_exceptions(self):
        with self.assertRaises(
            TypeError, msg="Non-integer systems should raise TypeError"
        ):
            offFreq._FrequencyOffset(self.ring, self.rf_params, 1.0)
        with self.assertRaises(
            TypeError,
            msg="Non-integer memebered iterables should raise TypeError",
        ):
...
class TestFixedFrequency(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fixed_frequency = offFreq.FixedFrequency(
            Ring=None,
            RFStation=None,
            FixedFrequency=None,
            FixedDuration=None,
...
class Test_FrequencyOffset(unittest.TestCase):
    ...
    def setUp(self):
        # TODO: implement test for `__init__`
        self.__frequency_offset = offFreq._FrequencyOffset(
            Ring=None, RFStation=None, System=None, MainH=None
        )

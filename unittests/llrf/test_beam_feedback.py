...
class TestBeamFeedback(unittest.TestCase):
    ...
    def test_SPS_RL(self):
        ...
        delta_tau = (
            beamAvgPos
            - (np.pi - self.rf_station.phi_rf[0, :-1])
        ...
        omega_eff = cmath.sqrt(
            -(PL_gain**2) + 4 * self.rf_station.omega_s0[0] ** 2
        )
        ...
        init_slope = (
            ...
            * (
                delta_tau[1]
                * omega_eff
                * np.exp(0.5 * PL_gain * time[1])
                / np.sin(0.5 * omega_eff * time[1])
                - delta_tau[0]
                * (PL_gain + omega_eff / np.tan(0.5 * omega_eff * time[1]))
            ).real
        )

        delta_tau_analytic = init_pos * np.exp(-0.5 * PL_gain * time)
        delta_tau_analytic *= (
            np.cos(0.5 * time * omega_eff).real
    ...
    def setUp(self):
        ...
        self.beam_feedback = BeamFeedback(
            Ring=None,
            RFStation=None,
            Profile=None,
            configuration=None,

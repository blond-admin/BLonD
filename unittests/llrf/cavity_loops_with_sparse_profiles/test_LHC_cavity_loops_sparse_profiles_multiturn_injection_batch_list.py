...
class TestLHCCavityLoopStandardProfile(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.beam, injected_batches = build_beam(self.ring, self.rf, 1)
...
class TestProfileEquivalence(unittest.TestCase):
    ...
    def test_bin_centers_match(self):
        for p, profile in enumerate(self.sparse.profiles_list):
            ...
            np.testing.assert_allclose(
                ...
                self.sparse.bin_centers[p * profile.n_slices],
                rtol=1e-12,
                atol=1e-12,
            )
    ...
    def test_bin_size_matches(self):
        self.assertAlmostEqual(
            self.standard.bin_size, self.sparse.bin_size, places=15
        )
...
class TestLHCCavityLoopConsistencyBetweenProfileTypesMultiTurnInjection(
    ...
    def test_fine_grid_cavity_response_inputs(self):
        ...
        np.testing.assert_equal(V_A_init, V_A_init_sparse)
        ...
        np.testing.assert_equal(I_gen_init, I_gen_init_sparse)
        np.testing.assert_allclose(
    ...
    def test_muliturn_injection_fine_grid_generator_current_consistent_fine_grid_disabled(self):
        common_kwargs = dict(
            f_c=self.f_c,
            G_gen=1,
            I_gen_offset=0.0,
        ...
        for k in range(number_of_batches - injected_batches):
            ...
            for p, profile in enumerate(self.CL_sparse.profile.profiles_list):
                ...
                np.testing.assert_allclose(
                    ...
                    self.CL_sparse.I_GEN_FINE[
                        p * profile.n_slices + 1: (
                                                              p + 1) * profile.n_slices + 1
                    ],
                    rtol=self.rtol,
                    atol=self.atol,
                    err_msg="I_GEN_FINE differs between standard Profile and "
    ...
    def test_muliturn_fine_grid_cavity_response_inputs(self):
        ...
        for k in range(number_of_batches - injected_batches):
            ...
            np.testing.assert_array_equal(self.CL_standard.rf_centers,
                                          self.CL_sparse.rf_centers)
            np.testing.assert_allclose(
                self.CL_standard.V_ANT_COARSE,
                self.CL_sparse.V_ANT_COARSE,
            ...
            np.testing.assert_allclose(V_A_init,
                                       V_A_init_sparse,
                                       atol = self.atol,
                                       rtol = self.rtol,
                                       )
            ...
            np.testing.assert_allclose(I_gen_init,
                                       I_gen_init_sparse,
                                       atol=self.atol,
                                       rtol=self.rtol,
                                       )

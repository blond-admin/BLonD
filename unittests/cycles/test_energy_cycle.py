...
class TestEnergyCycleBase(unittest.TestCase):
    def setUp(self):
        ...
        self.magnetic_cycle_base = MagneticCycleBaseHelper(
            reference_particle=proton,
            magnetic_rigidity_init=self.momentum_init / (proton.charge * c0),
        )
...
class TestEnergyCyclePerTurnAllCavities(unittest.TestCase):
    ...
    def test_headless(self):
        ecptac = MagneticCyclePerTurnAllRFStations.headless(
            value_init=10,
    ...
    def test_get_target_total_energy(self):
        cycle = MagneticCyclePerTurnAllRFStations.headless(
            value_init=10,
            values_after_rf_station_per_turn=np.ones((2, 20)),
            reference_particle=uranium_29,
        )

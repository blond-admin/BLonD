...
class testCoastingBeamModule(unittest.TestCase):
    ...
    def test_defaults(self):
        ...
        self.assertAlmostEqual(
            np.min(self.beam.dt) * 1e9,
            0,
            places=2,
            msg="Min dt should be almost 0",
        )
        self.assertAlmostEqual(
            np.max(self.beam.dt) * 1e9,
            self.ring.t_rev[0] * 1e9,
            places=2,
        ...
        self.assertAlmostEqual(
            np.mean(diff), 2.4, delta=0.1, msg="Default not Gaussian enough"
        )

        self.assertAlmostEqual(
            np.std(diff), 86, delta=1, msg="Default not Gaussian enough"
        )
        ...
        self.assertAlmostEqual(
            np.mean(vals), 6666, delta=1, msg="Default not flat enough"
        )
        self.assertAlmostEqual(
            np.std(vals), 80, delta=1, msg="Default not flat enough"
        )

    def test_spread_types(self):
        testTypes = ["dp/p", "dE/E", "dp", "dE"]
        for t in testTypes:
            cBeam.generate_coasting_beam(
                self.beam, 0, self.ring.t_rev[0], spread_type=t
            )

        with self.assertRaises(
            blond_exceptions.DistributionError,
            msg="""Invalid spread type 
                               should raise exception""",
        ):
            cBeam.generate_coasting_beam(
                self.beam, 0, self.ring.t_rev[0], spread_type="bad spread"
            )
    ...
    def test_distribution_types(self):
        ...
        for t in testDists:
            cBeam.generate_coasting_beam(
                self.beam, 0, self.ring.t_rev[0], distribution=t
            )
        ...
        with self.assertRaises(
            ...
            cBeam.generate_coasting_beam(
                self.beam, 0, self.ring.t_rev[0], spread_type="user"
            )
    ...
    def test_spread_offset(self):
        ...
        for s in spreads:
            for o in offsets:
                ...
                self.assertAlmostEqual(
                    cents[np.where(vals == np.max(vals))[0][0]],
                    o,
                    delta=2.5 * (cents[1] - cents[0]),

...
def setup_blond2():
    ...
    i_beam_blond2 = np.zeros((n_turns, CL.n_coarse), dtype=complex)
    rf_voltage_blond2 = np.zeros((n_turns, CL.n_coarse), dtype=complex)
    line_density_blond2 = np.zeros((n_turns, profile.n_slices))

    for i in range(n_turns):
        profile.track()
        tracker.track()
        rf_power_blond2[i, :] = CL.generator_power()[-CL.n_coarse :]
        i_beam_blond2[i, :] = CL.I_BEAM_COARSE[-CL.n_coarse :]
        rf_voltage_blond2[i, :] = CL.V_ANT_COARSE[-CL.n_coarse :]
        line_density_blond2[i, :] = profile.n_macroparticles

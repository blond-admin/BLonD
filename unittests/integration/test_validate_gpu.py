...
gpu_out_dir = os.path.join(
    this_directory, "../../__EXAMPLES/gpu_output_files/"
)
...
class TestGPUCorrectness(unittest.TestCase):
    def _testFilesClose(self, example, rtol=1e-10, atol=0):
        ...
        gpu_file = os.path.join(
            gpu_out_dir, "{}_test_data.txt".format(example)
        )
        ...
        if os.path.isfile(file) and os.path.isfile(gpu_file):
            ...
            if len(data) == 0:
                raise unittest.SkipTest(
                    "[{}] Non-GPU test file empty".format(example)
                )
            ...
            if len(datagpu) == 0:
                raise unittest.SkipTest(
                    "[{}] GPU test file empty".format(example)
                )

...
mpi_out_dir = os.path.join(
    this_directory, "../../__EXAMPLES/mpi_output_files/"
)
...
class TestMPICorrectness(unittest.TestCase):
    def _testFilesClose(self, example, rtol=1e-10, atol=0):
        ...
        mpi_file = os.path.join(
            mpi_out_dir, "{}_test_data.txt".format(example)
        )
        ...
        if os.path.isfile(file) and os.path.isfile(mpi_file):
            ...
            if len(data) == 0:
                raise unittest.SkipTest(
                    "[{}] Non-MPI test file empty".format(example)
                )
            ...
            if len(datampi) == 0:
                raise unittest.SkipTest(
                    "[{}] MPI test file empty".format(example)
                )
            ...
            np.testing.assert_equal(
                header,
                headermpi,
                err_msg="[{}] The headers of the test files disagree".format(
                    example
                ),
            )
            ...
            for idx, row, row_mpi in zip(np.arange(len(data)), data, datampi):
                for h, x, xmpi in zip(header, row, row_mpi):
                    np.testing.assert_allclose(
                        ...
                        atol=atol,
                        rtol=rtol,
                        err_msg="[{}] Test failed in row {}, column {}".format(
                            example, idx, h
                        ),
                    )
        else:

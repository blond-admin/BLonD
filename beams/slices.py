...
class Slices(object):
    ...
    def slice_constant_space(self, bunch):
        ...
        self.first_index_in_bin = np.searchsorted(bunch.z[:n_macroparticles_alive], self.z_bins)
    ...
    def slice_constant_charge(self, bunch):
        ...
        self.n_cut_tail = np.searchsorted(bunch.z[:n_macroparticles_alive], self.z_cut_tail)
        self.n_cut_head = n_macroparticles_alive - np.searchsorted(bunch.z[:n_macroparticles_alive], self.z_cut_head) 
        ...
        z_bins_internal = 1/2 * (bunch.z[indexes] + bunch.z[indexes-1])

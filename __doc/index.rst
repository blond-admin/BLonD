.. BLonD documentation master file, created by
   sphinx-quickstart on Fri Mar 10 19:37:27 2017.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to the BLonD documentation!
===================================

Contents
========
The Beam Longitudinal Dynamics code BLonD is a CERN software package for the
simulatation of longitudinal beam dynamics in synchrotrons.

Visit the BLonD website_ for more details and the github_ distribution to 
download the code.

.. _website: http://blond.web.cern.ch/
.. _github: https://github.com/blond-admin/BLonD/releases

Modules
=======
=======
Welcome to BLonD's documentation!
=================================

Contents:
=========

What is real?
-------------

How do you define real?

    You take the blue pill —the story ends, you wake up in your bed and believe
    whatever you want to believe. You take the red pill— you stay in Wonderland,
    and I show you how deep the rabbit hole goes. Remember: all I'm offering is
    the truth. Nothing more.

Beam Longitudinal Dynamics is fun
----------------------------------

But be careful, the cake is a lie !

More undefined quote?
---------------------

    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur tempor
    hendrerit ligula a bibendum. Praesent sed orci enim.
    Morbi ac elementum magna. Nunc vitae sodales velit, in euismod ligula.
    Phasellus bibendum erat vitae tortor facilisis convallis. Integer consectetur,
    erat id pretium tincidunt, nisl diam tincidunt leo, nec aliquam ante orci vitae
    tortor. Nullam euismod, massa sed eleifend pellentesque, quam velit congue arcu,
    et eleifend justo augue non erat. Cras ut sapien interdum, lobortis velit ac,
    vestibulum est. Praesent id bibendum lacus. Quisque tempor diam in rutrum tempor.
    Vivamus imperdiet dui nisi, eget egestas nibh placerat non. Sed sapien risus,
    maximus id nisl non, dapibus blandit nulla. Suspendisse eget orci tortor.
    Nunc finibus auctor ligula a pellentesque.

Modules


.. toctree::
    :maxdepth: 2

    source/beams.rst
    source/impedances.rst
    source/input_parameters.rst
    source/llrf.rst
    source/monitors.rst
    source/plots.rst
    source/synchrotron_radiation.rst
    source/toolbox.rst
    source/trackers.rst
    source/setup_cpp.rst



Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

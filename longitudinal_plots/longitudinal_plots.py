...
def plot_long_phase_space(beam, nturns, xmin, xmax, ymin, ymax, 
    ...
    if xunit == None or xunit == 'rad':
        ...
        print("WARNING: unit of plot_long_phase_space not recognized!")         
...
def plot_bunch_length_evol(beam, h5file, nturns, unit=None):
    ...
    # Get 4-sigma bunch length data in different units
    t = range(1, nturns + 1) 
    ...
    if unit == None or unit == 'rad':
        ...
        bl *= 4.e9/c/beam.ring.beta_i(beam)*beam.ring.radius 
    elif unit == 'm':
        bl *= 4*beam.ring.radius 
    else:
        print("WARNING: unit of plot_bunch_length_evol not recognized!")   
    ...
    if unit == None or unit == 'rad':
        ax.set_ylabel (r"Bunch length, $\theta_{4\sigma}$ r.m.s. [rad]")
        ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    elif unit == 'ns':
        ax.set_ylabel (r"Bunch length, $\tau_{4\sigma}$ r.m.s. [ns]")
    elif unit == 'm':
        ax.set_ylabel (r"Bunch length, $z_{4\sigma}$ r.m.s. [m]")
...
def plot_bunch_length_evol_gaussian(beam, h5file, nturns, unit=None):
    ...
    if unit == 'ns':
        bl *= 1.e9/c/beam.ring.beta_i(beam)*beam.ring.radius 
    elif unit == 'm':
        bl *= beam.ring.radius 
    else:
        print("WARNING: unit of plot_bunch_length_evol_gaussian not recognized!")   
    ...
    ax.set_xlabel(r"No. turns [T$_0$]")
    if unit == None or unit == 'rad':
        ax.set_ylabel (r"Bunch length, $\theta_{4\sigma}$ Gaussian fit [rad]")
        ax.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    elif unit == 'ns':
        ax.set_ylabel (r"Bunch length, $\tau_{4\sigma}$ Gaussian fit [m]")
    elif unit == 'm':
        ax.set_ylabel (r"Bunch length, $z_{4\sigma}$ Gaussian fit [ns]")

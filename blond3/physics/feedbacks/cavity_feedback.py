...
from .helpers import (
    ...
    comb_filter,
    moving_average,
    get_power_gen_i, fir_filter_lhc_otfb_coeff, cavity_response_sparse_matrix,
    smooth_step,
)
...
if TYPE_CHECKING:
    from typing import Optional, Any, Optional as LateInit
...
class BirksCavityFeedback(LocalFeedback):
    ...
    def __init__(
        ...
        n_periods_corase
            Number of periods for the corase grid
        harmonic_index
            Index of the RF harmonic that should be controlled by the feedback
        use_lowpass_filter
            Whether to apply a lowpass filter whne calculating the beam current
        section_index
        ...
        self.n_periods_corase = int(n_periods_corase)

        self.T_s = (self.n_periods_corase * 2 * np.pi) / self._parent_cavity._omega[
            self.harmonic_index
        ]
        # TODO REMWORK/REMOVE
        t_rev = float(
        ...
        self.omega_carrier = (
            self._parent_cavity._omega[self.harmonic_index] / self.n_periods_corase
        )
    ...
    def update_rf_variables(self):
        ...
        self.omega_carrier = self.omega_rf / self.n_periods_corase

        # Present sampling time
        self.T_s_prev = self.T_s
        self.T_s = self.n_periods_corase * 2 * np.pi / self.omega_rf
        ...
        self.rf_centers = (
            np.arange(self.n_coarse) + 0.5 / self.n_periods_corase
        ) * self.T_s + self.dT
    ...
    def track_no_beam(self, n_pretrack: Optional[int] = 1):
        r"""
    ...
    def rf_beam_current(self, use_lowpass_filter: bool = False):
...
class SPSOneTurnFeedback(BirksCavityFeedback):
    ...
    def __init__(
        ...
        self.set_parent_cavity(
            _parent_cavity
        )  # FIXME remove, should be handled elsewhere
        super().__init__(
            profile=profile,

...
contains
  ...
  function beam_phase(bin_centers, profile, alpha, omega_rf, phi_rf, bin_size, n_bins) result(phase)
    ...
    real(kind=4), intent(in) :: bin_centers(n_bins), profile(n_bins)
    real(kind=4), intent(in) :: alpha, omega_rf, phi_rf, bin_size
    real(kind=4) :: base(n_bins), array1(n_bins), array2(n_bins)
    real(kind=4) :: scoeff, ccoeff, phase
    integer :: i

...
from blond3.legacy.blond2.utils import precision
from blond3.legacy.blond2.utils.custom_warnings import PerformanceWarning
...
class RingAndRFTracker:
        ...
        if self.cavityFB is not None:
            ...
            for ind, feedback in enumerate(self.cavityFB):
                if feedback is not None:
                    ...
                    self.rf_voltage += bm.rf_volt_comp(voltages[ind:ind + 1],
                                                       omega_rf[ind:ind + 1],
                                                       phi_rf[ind:ind + 1],
                                                       self.profile.bin_centers)
    ...
    def track(self):
        ...
        self.rf_params.dphi_rf += (2. * np.pi

...
from blond.legacy.blond2.trackers.utilities import total_voltage
...
rf_params_1 = RFStation(general_params, [h], [V1], [dphi], section_index=1)
long_tracker_1 = RingAndRFTracker(rf_params_1, beam)

rf_params_2 = RFStation(general_params, [h], [V2], [dphi], section_index=2)
...
bigaussian(
    general_params, rf_params_tot, beam, tau_0 / 4, reinsertion="on", seed=1
)
...
# PLOTS
format_options = {
    "dirname": this_directory + "../gpu_output_files/EX_04_fig",
    "linestyle": ".",
}
plots = Plot(
    general_params,
...
# Accelerator map
map_ = (
    [long_tracker_1] + [long_tracker_2] + [slice_beam]
)  # + [bunchmonitor] + \
# [plots]
print("Map set")
print("")
...
with open(
    this_directory + "../gpu_output_files/EX_04_test_data.txt", "w"
) as f:
    f.write(test_string)

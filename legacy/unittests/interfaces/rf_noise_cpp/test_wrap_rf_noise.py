...
def rf_noise_repo_is_missing():
    try:
        from blond.legacy.blond2.interfaces.rf_noise_cpp.wrap_rf_noise import (
            rf_noise,
            _local_path,
...
class RfNoise(unittest.TestCase):
    ...
    def test_flat_noise(self):
        ...
        self.assertTrue(
            np.all(
                (freq_masked > (f_low[0] - delta_x))
                & (freq_masked < (f_high[0] + delta_x))
            )
        )
    ...
    def test_docstring_example(self):
        ...
        plt.specgram(
            results,
            Fs=11245.49,
            NFFT=int(20000000 / 1000),
            label="specgram(results)",
        )

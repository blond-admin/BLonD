...
# BLonD imports
# --------------
from blond.legacy.blond2.beam.beam import Beam, Electron, Particle, Proton
...
class testParticleClass(unittest.TestCase):
    ...
    def test_particle_attributes(self):
        for attribute in ["mass", "charge", "radius_cl", "c_gamma", "c_q"]:
            self.assertTrue(
                hasattr(self.test_particle, attribute),
    ...
    def test_attribute_types(self):
        for attribute in ["mass", "charge"]:
            self.assertIsInstance(
                getattr(self.test_particle, attribute),
                float,
...
class testElectron(unittest.TestCase):
    ...
    def test_classical_electron_radius(self):
        self.assertAlmostEqual(
            self.electron.radius_cl,
            physical_constants["classical electron radius"][0],
            delta=1e-24,
...
class testBeamClass(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Define RF section
        # -----------------
        self.rf_params = RFStation(self.general_params, [4620], [7e6], [0.0])
    ...
    def test_variables_types(self):
        self.assertIsInstance(
            self.beam.beta, float, msg="Beam: beta is not a float"
        )
        self.assertIsInstance(
            self.beam.gamma, float, msg="Beam: gamma is not a float"
        )
        self.assertIsInstance(
            self.beam.energy, float, msg="Beam: energy is not a float"
        )
        self.assertIsInstance(
            self.beam.momentum, float, msg="Beam: momentum is not a float"
        )
        self.assertIsInstance(
            self.beam.mean_dt, float, msg="Beam: mean_dt is not a float"
        )
        self.assertIsInstance(
            self.beam.mean_dE, float, msg="Beam: mean_dE is not a float"
        )
        self.assertIsInstance(
            self.beam.sigma_dt, float, msg="Beam: sigma_dt is not a float"
        )
        self.assertIsInstance(
            self.beam.sigma_dE, float, msg="Beam: sigma_dE is not a float"
        )
        self.assertIsInstance(
            self.beam.intensity, float, msg="Beam: intensity is not a float"
        )
        self.assertIsInstance(
        ...
        self.assertIsInstance(
            self.beam.ratio, float, msg="Beam: ratio is not a float"
        )
        self.assertIsInstance(
            self.beam.id, np.ndarray, msg="Beam: id is not a numpy.array"
        )
        self.assertIn(
            "int",
            type(self.beam.id[0]).__name__,
        ...
        self.assertIsInstance(
            self.beam.dt, np.ndarray, msg="Beam: dt is not a numpy.array"
        )
        self.assertIsInstance(
            self.beam.dE, np.ndarray, msg="Beam: dE is not a numpy.array"
        )
        self.assertIn(
            "float",
            type(self.beam.dt[0]).__name__,
            msg="Beam: dt does not contain float",
        )
        self.assertIn(
            "float",
            type(self.beam.dE[0]).__name__,
    ...
    def test_losses_separatrix(self):
        ...
        try:
            ...
            self.skipTest(
                "Skipped because of known bug in deepcopy. Exception message %s"
                % str(te)
            )
        self.beam.losses_separatrix(self.general_params, self.rf_params)
        self.assertEqual(
            len(self.beam.id[self.beam.id == 0]),
            0,
    ...
    def test_losses_longitudinal_cut(self):
        ...
        self.beam.losses_longitudinal_cut(0.0, 5e-9)
        self.assertEqual(
            len(self.beam.id[self.beam.id == 0]),
            0,
    ...
    def test_addition(self):
        ...
        self.assertEqual(
            self.beam.id[2100000:2100100].tolist(),
            [0] * 100,
            msg="particle ids not applied correctly",
        )

        self.assertEqual(
            self.beam.id[-1], 2100200,
                         msg='particle ids not applied correctly')
        ...
        with self.assertRaises(
            blond_exceptions.ParticleAdditionError,
        ...
        with self.assertRaises(
            TypeError, msg="Wrong type should raise exception"
        ):
    ...
    def test_init_de_dt(self):
        ...
        testdts = np.linspace(0, 10e-9, 2000000)
        beam = Beam(
            ring=self.general_params,
            n_macroparticles=int(2000000),
            intensity=1e9,
            dE=testdEs,
            dt=testdts,
        )
    ...
    def test_init_de_dt2(self):
        ...
        self.assertRaises(
            ...
            lambda: Beam(
                ring=self.general_params,
                n_macroparticles=int(1000000),  # mismatch in n_macroparticles
                intensity=1e9,
                dE=testdEs,
                dt=testdts,
            ),
    ...
    def test_change_id(self):
        ...
        self.assertEqual(
            n_macroparticles_alive_after, n_macroparticles_alive_before - 1
        )
        self.assertEqual(
            n_macroparticles_not_alive_after,
            n_macroparticles_not_alive_before + 1,
        )

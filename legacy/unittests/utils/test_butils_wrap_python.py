...
from blond.legacy.blond2.beam.sparse_profiles import SparseBucket
...
from blond.legacy.blond2.impedances.impedance import InducedVoltageResonator
...
from blond.legacy.blond2.utils.butils_wrap_python import (
...
class Test:
    ...
    @pytest.mark.parametrize(
        "n_particles,n_slices", [(125421, 17), (100000, 100)]
    )
    ...
    @pytest.mark.parametrize(
        "n_particles,n_kicks", [(10, 1), (10000, 4), (100000, 2)]
    )
    ...
    @pytest.mark.parametrize(
        ...
        [(100, 1, 1), (100, 4, 10), (100000, 1, 10), (100000, 10, 10)],
    )
    ...
    @pytest.mark.parametrize(
        "n_particles,n_iter", [(100, 1), (100, 10), (100000, 1), (100000, 10)]
    )
    ...
    @pytest.mark.parametrize(
        ...
        [(100, 16, 10), (10000, 256, 10), (100000, 100, 10)],
    )
    ...
    def test_slice_smooth(self, n_particles, n_slices, cut_left, cut_right):
        ...
        np.testing.assert_array_almost_equal(profile_py, profile, decimal=8)

    @pytest.mark.parametrize(
        "n_particles,n_kicks,seed",
        [(1000, 1, 1234), (10000, 10, 1), (100000, 10, 0)],
    )
    ...
    def test_set_random_seed_not_equal(self, n_particles, n_kicks, seed):
        ...
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_almost_equal, dE, dE_2
        )
    ...
    def test_fast_resonator(self, size):
        ...
        np.testing.assert_array_almost_equal(
            impedance, impedance_py, decimal=8
        )
...
class TestWithObjects:
    ...
    @pytest.mark.parametrize(
        "n_bunches,n_slices,bunch_spacing",
        [(1, 16, 2), (8, 64, 5), (72, 128, 10)],
    )
    def test_sparse_histo(self, n_bunches, n_slices, bunch_spacing):
        ...
        for i in range(int(np.sum(filling_pattern))):
            self.beam.dt[
                indexes[
                    int(i * len(self.beam.dt) // np.sum(filling_pattern))
                ] : indexes[
                    int(
                        (i + 1) * len(self.beam.dt) // np.sum(filling_pattern)
                        - 1
                    )
                ]
        ...
        np.testing.assert_array_almost_equal(
            slice_beam_py.n_macroparticles_array,
            slice_beam_cpp.n_macroparticles_array,
            decimal=8,
        )
    ...
    def test_music_track(self, n_macroparticles):
        ...
        music_cpp = Music(
            beam, resonator, n_macroparticles, self.N_b, self.ring.t_rev[0]
        )
    ...
    def test_resonator_induced_voltage_1_turn(self, n_macroparticles):
        ...
        general_params_res = Ring(
            self.C, self.alpha, self.p_i, Proton(), self.N_t
        )

        RF_sct_par_res = RFStation(
            general_params_res,
            [self.h],
            [voltage_program],
            [phi_offset],
        ...
        cut_options_res = CutOptions(
            cut_left=0,
            cut_right=2 * np.pi,
            n_slices=number_slices,
            RFSectionParameters=RF_sct_par_res,
            cuts_unit="rad",
        )
        slice_beam_res = Profile(
            self.beam, cut_options_res, FitOptions(fit_option="gaussian")
        )
        ...
        bm.use_numba()
        ind_volt_res = InducedVoltageResonator(
            self.beam, slice_beam_res, resonator
        )
        ...
        np.testing.assert_array_almost_equal(
            volt_save, ind_volt_res.induced_voltage, decimal=8
        )
        np.testing.assert_array_almost_equal(
            time_save, ind_volt_res._deltaT, decimal=8
        )

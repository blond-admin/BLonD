...
from blond3.legacy.blond2.beam.profile import Profile, CutOptions, FitOptions
from blond3.legacy.blond2.llrf.cavity_feedback import LHCCavityLoop, LHCCavityLoopCommissioning
from blond3.legacy.blond2.llrf.transfer_function import TransferFunction
...
logging.info('CLOSED LOOP, no excitation, 1000 turn tracking')
CL = LHCCavityLoop(
    rf, profile, f_c=rf.omega_rf[0, 0] / (2 * np.pi), G_gen=1,
    I_gen_offset=0, n_cavities=8, n_pretrack=1000, Q_L=20000,
    R_over_Q=45, tau_loop=650e-9,
    RFFB=LHCCavityLoopCommissioning(open_loop=False, G_a=6.8e-6, G_d=10, excitation=False)
)
...
logging.info('CLOSED LOOP, with excitation, 1000 turns tracking')
CL = LHCCavityLoop(
    rf, profile, f_c=rf.omega_rf[0, 0] / (2 * np.pi), G_gen=1,
    I_gen_offset=0, n_cavities=8, n_pretrack=1000, Q_L=20000,
    R_over_Q=45, tau_loop=650e-9,
    RFFB=LHCCavityLoopCommissioning(open_loop=False, G_a=6.8e-6, G_d=10, excitation=True)
)

...
from blond.legacy.blond2.llrf.signal_processing import feedforward_filter
...
if FILTER_DESIGN:
    ...
    logging.info(
        "3-section cavity filter design with modified filling time"
        + " and fixed n_taps"
    )
...
if FEEDFORWARD:
    ...
    for i in range(n_bunches):
        beam.dt[int(i * N_m) : int((i + 1) * N_m)] = (
            bunch.dt + (i + 10) * rf.t_rf[0, 0] * bunch_spacing
        )
    ...
    tracker = RingAndRFTracker(
        rf, beam, cavity_feedback=OTFB, interpolation=True, profile=profile
    )
    ...
    ax1_1.fill_between(
        1e6 * profile.bin_centers,
        0,
        1e-6 * OTFB.V_sum.real,
        alpha=0.2,
        color="grey",
    )
    ...
    for i in range(N_t):
        ...
        ax1_1.plot(
            1e6 * profile.bin_centers, 1e-6 * OTFB.V_sum.real, color=colors[i]
        )
        ax1_1.fill_between(
            1e6 * profile.bin_centers,
            0,
        ...
        ax1_2.plot(
            1e6 * profile.bin_centers, 1e-6 * OTFB.V_sum.imag, color=colors[i]
        )
        ax1_2.fill_between(
            1e6 * profile.bin_centers,
            0,
        ...
        ax2_1.plot(
            1e6 * profile.bin_centers,
            1e-6 * OTFB.OTFB_1.V_IND_FINE_BEAM.real
            + 1e-6 * OTFB.OTFB_2.V_IND_FINE_BEAM.real,
            color=colors[i],
        )
        ax2_1.fill_between(
            1e6 * profile.bin_centers,
        ...
        ax2_2.plot(
            1e6 * profile.bin_centers,
            1e-6 * OTFB.OTFB_1.V_IND_FINE_BEAM.imag
            + 1e-6 * OTFB.OTFB_2.V_IND_FINE_BEAM.imag,
            color=colors[i],
        )
        ax2_2.fill_between(
            1e6 * profile.bin_centers,

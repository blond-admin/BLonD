...
from blond.legacy.blond2.llrf.impulse_response import (SPS3Section200MHzTWC,
                                                       SPS4Section200MHzTWC,
)
from blond.legacy.blond2.llrf.signal_processing import rf_beam_current
...
profile = Profile(
    beam,
    cut_options=CutOptions(cut_left=-1.0e-9, cut_right=6.0e-9, n_slices=100),
)
profile.track()
...
if RF_CURRENT:
    ...
    rf_current = rf_beam_current(
        profile, 2 * np.pi * f_rf, ring.t_rev[0], lpf=False
    )
    rf_current_filt = rf_beam_current(
        profile, 2 * np.pi * f_rf, ring.t_rev[0], lpf=True
    )
    ...
    ax4 = ax3.twinx()
    ax4.plot(
        profile.bin_centers, rf_current_filt.real, "b", label="filtered, real"
    )
    ax4.plot(
        profile.bin_centers, rf_current_filt.imag, "r", label="filtered, imag"
    )
...
if RF_CURRENT2:
    ...
    tot_charges = (
        np.sum(profile2.n_macroparticles)
        / beam2.n_macroparticles
        * beam2.intensity
    )
    logging.info(
        "Total number of charges %.10e p"
        % (
            np.sum(profile2.n_macroparticles)
            / beam2.n_macroparticles
            * beam2.intensity
    ...
    ax6 = ax5.twinx()
    ax6.plot(
        profile2.bin_centers * 1e6,
        rf_current_fine.real,
        "b",
        label="fine, real",
    )
    ax6.plot(
        profile2.bin_centers * 1e6,
        rf_current_fine.imag,
        "r",
        label="fine, imag",
    )
    ...
    logging.info(
        "Peak beam current, theor %.4f A" % (2 * N_b * e / bunch_spacing)
    )
    fig.tight_layout()
...
if IMP_RESP:
    ...
    time = np.linspace(-1e-6, 4.0e-6, 10000)
    ...
    plt.plot(
        t_gen, TWC_v1.h_gen.real, "purple", marker=".", label="hs_gen, OTFB"
    )
    plt.xlabel("Time [s]")
...
if FINE_COARSE:
    ...
    rf_current_fine, rf_current_coarse = rf_beam_current(
        ...
        lpf=False,
        downsample={
            "Ts": rf.t_rev[0] / rf.harmonic[0, 0],
            "points": rf.harmonic[0, 0],
        },
    )
    ...
    plt.plot(
        ...
        "b",
        marker=".",
        label="h_beam, fine, real",
    )
    plt.plot(
        time_fine * 1e6,
        OTFB.TWC.h_beam.real,
        "b",
        marker=".",
        alpha=0.5,
        label="h_beam, fine, real",
    ...
    plt.plot(
        ...
        "teal",
        marker=".",
        label="h_beam, coarse, real",
    )
    plt.plot(
        time_coarse * 1e6,
        OTFB.TWC.h_beam_coarse.real,
        "teal",
        marker=".",
        alpha=0.5,
        label="h_beam, coarse, real",
    ...
    plt.plot(
        ...
        "r",
        marker=".",
        label="h_beam, fine, imag",
    )
    plt.plot(
        time_fine * 1e6,
        OTFB.TWC.h_beam.imag,
        "r",
        marker=".",
        alpha=0.5,
        label="h_beam, fine, imag",
    ...
    plt.plot(
        ...
        "orange",
        marker=".",
        label="h_beam, coarse, imag",
    )
    plt.plot(
        time_coarse * 1e6,
        OTFB.TWC.h_beam_coarse.imag,
        "orange",
        marker=".",
        alpha=0.5,
        label="h_beam, coarse, imag",
    ...
    plt.plot(
        time_fine * 1e6,
        V_beam_fine.real * 1e-6,
        "b",
        marker=".",
        label="V_beam, fine, real",
    )
    plt.plot(
        time_fine * 1e6,
        OTFB.V_IND_FINE_BEAM[-OTFB.profile.n_slices :].real * 1e-6,
        "b",
        marker=".",
        alpha=0.5,
        label="V_beam, fine, real",
    ...
    plt.plot(
        time_coarse * 1e6,
        V_beam_coarse.real * 1e-6,
        "teal",
        marker=".",
        label="V_beam, coarse, real",
    )
    plt.plot(
        time_coarse * 1e6,
        OTFB.V_IND_COARSE_BEAM[-h[0] :].real * 1e-6,
        "teal",
        marker=".",
        alpha=0.5,
        label="V_beam, coarse, real",
    ...
    plt.plot(
        time_fine * 1e6,
        V_beam_fine.imag * 1e-6,
        "r",
        marker=".",
        label="V_beam, fine, imag",
    )
    plt.plot(
        time_fine * 1e6,
        OTFB.V_IND_FINE_BEAM[-OTFB.profile.n_slices :].imag * 1e-6,
        "r",
        marker=".",
        alpha=0.5,
        label="V_beam, fine, imag",
    ...
    plt.plot(
        time_coarse * 1e6,
        V_beam_coarse.imag * 1e-6,
        "orange",
        marker=".",
        label="V_beam, coarse, imag",
    )
    plt.plot(
        time_coarse * 1e6,
        OTFB.V_IND_COARSE_BEAM[-h[0] :].imag * 1e-6,
        "orange",
        marker=".",
        alpha=0.5,
        label="V_beam, coarse, imag",
...
if VIND_BEAM:
    profile = Profile(
        beam,
        cut_options=CutOptions(
            cut_left=-1.0e-9, cut_right=6.0e-9, n_slices=140
        ),
    )
    profile.track()
    ...
    OTFB_3 = SPSOneTurnFeedback(
        rf, profile, 3, commissioning=SPSCavityLoopCommissioning(open_ff=True)
    )
    OTFB_4 = SPSOneTurnFeedback(
        rf, profile, 4, commissioning=SPSCavityLoopCommissioning(open_ff=True)
    )
    OTFB_5 = SPSOneTurnFeedback(
        rf, profile, 5, commissioning=SPSCavityLoopCommissioning(open_ff=True)
    )
    ...
    OTFB_5.track()
    V_ind_beam = (
        OTFB_3.V_IND_FINE_BEAM[-OTFB_3.profile.n_slices :]
        + OTFB_4.V_IND_FINE_BEAM[-OTFB_4.profile.n_slices :]
        + OTFB_5.V_IND_FINE_BEAM[-OTFB_5.profile.n_slices :]
    )
    plt.figure()
    convtime = np.linspace(
        -1e-9,
        -1e-9 + len(V_ind_beam.real) * profile.bin_size,
        len(V_ind_beam.real),
    )
    plt.plot(convtime, V_ind_beam.real, "b--")
    plt.plot(
        convtime[:140], V_ind_beam.real[:140], "b", label="Re(Vind), OTFB"
    )
    plt.plot(convtime, V_ind_beam.imag, "r--")
    plt.plot(
        convtime[:140], V_ind_beam.imag[:140], "r", label="Im(Vind), OTFB"
    )
    plt.plot(
        convtime[:140],
    ...
    indVoltageTWC = InducedVoltageTime(
        beam, profile, [TWC200_4, TWC200_4, TWC200_5, TWC200_5]
    )
    indVoltage = TotalInducedVoltage(beam, profile, [indVoltageTWC])
    indVoltage.induced_voltage_sum()
    plt.plot(
        indVoltage.time_array,
        indVoltage.induced_voltage,
        color="limegreen",

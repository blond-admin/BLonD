...
CL = LHCCavityLoop(
    ...
    RFFB=LHCCavityLoopCommissioning(
        open_loop=False, G_a=6.8e-6, G_d=10, excitation=False
    ),
)
logging.info(
...
CL = LHCCavityLoop(
    ...
    tau_loop=650e-9,
    RFFB=LHCCavityLoopCommissioning(
        open_loop=False, G_a=6.8e-6, G_d=10, excitation=True
    ),
)
...
ax1.plot(
    TF60k.f_est / 10**6,
    20 * np.log10(np.abs(TF60k.H_est)),
    "r",
    linewidth=0.3,
    label="60k",
)
...
ax2.plot(
    TF.f_est / 10**6,
    (180 / np.pi) * np.unwrap(np.angle(TF.H_est)),
    "b",
    linewidth=0.3,
    label="20k",
)
ax2.plot(
    TF60k.f_est / 10**6,
    (180 / np.pi) * np.unwrap(np.angle(TF60k.H_est)),
    "r",
    linewidth=0.3,
    label="60k",
)
...
ax1.set_title("Transfer function")
ax1.plot(
    TF60k.f_est / 10**3, 20 * np.log10(np.abs(TF60k.H_est)), "b", linewidth=0.3
)
